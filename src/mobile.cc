...
// Hiding bit added by Sphenotus Sept 6, 2000

  assign_bit( &mob->status, STAT_HIDING,
    is_set( mob->affected_by, AFF_HIDE ) );
...
void load_mobiles( void )
{
  ...
  if( !strcmp( header, "#MOBILES2" ) )
    version = 2;
  else
    if( strcmp( header, "#MOBILES" ) ) 
  ...
  for( ; ; ) {
    ...
    if( version >= 2 )
    {
      char* race = fread_string( fp, MEM_DESCR );
      bool found = FALSE;
      
      for( int i = 0; i < MAX_ENTRY_RACE; i++ )
        if( !strcmp( race, race_table[i].name ) )
        {
          shdata->race = i;
          found = TRUE;
          break;
      ...
      if( !found )
        panic( "Load_mobiles: Invalid race. [%d]", species->vnum );
      
      free_string( race, MEM_DESCR );
    }
