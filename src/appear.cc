...
const char* appearance_string = "Your %s now been set.  When you have\
 completed your %s and your description please type request avatar.  See\
 help appearance and the identity command for more information.\n\r";
...
void do_appearance( char_data* ch, char* argument )
{
  ...
  *argument = tolower( *argument );
  ...
  fsend( ch, appearance_string, "appearance has", "keywords" );
}
...
void do_keywords( char_data* ch, char* argument )
{
  ...
  fsend( ch, appearance_string, "keywords have", "appearance" );
}
...
void do_request( char_data* ch, char* argument )
{
  ...
  if( matches( argument, "immortal" ) ) {
    ...
    sprintf( tmp = static_string( ),
      ...
      ch->Name( ), argument );
    info( empty_string, LEVEL_APPRENTICE, tmp,
       IFLAG_REQUESTS, 1, ch );
    return;
    }

  if( is_avatar( ch ) || is_immortal( ch ) ) {
    if( matches( argument, "remove" ) ) {
      victim = one_character( ch, argument, "request remove", (thing_array*) &player_list );
      if( victim == NULL )
        return;
      if( remove( request_app, victim ) | remove( request_imm, victim ) )
        send( ch, "Requests for %s cancelled.\n\r", victim->descr->name );
      else
        send( ch, "%s has no pending requests.\n\r", victim->descr->name ); 
      return;   
