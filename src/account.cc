...
account_data* find_account( char* name, bool all )
{
  ...
  return( pos != max_account
    ...
    ? account_list[pos] : (account_data *) NULL );
}
...
bool valid_acnt_name( char* argument )
{
  ...
  for( letter = argument; *letter != '\0'; letter++ ) {
     if( *letter == ' ' )
       return FALSE;
...
{  
  ...
  if( change ) {
    fprintf( fp, "A request was entered at The Summers of Faldyor to change\n");
    ...
    fprintf( fp, "A request was entered at The Summers of Faldyor to open an\n" );
    fprintf( fp, "account at this email address.  If this is indeed\n" );
    fprintf( fp, "the case your confirmation code is '%s'.\n", confirm );
    fprintf( fp, "The request was from a user connected from %s.\n", 
      link->host );
    
     fp2 = open_file( FILE_DIR, "iplog.txt", "a" );
     if( fp2 == NULL ) {
       roach( "Could not open ip log file" );
       }
     else {
       fprintf( fp2,  "%s created account %s at %s", link->host,
account->name,
(char*) ctime( &current_time ) );
  ...
  sprintf( tmp,
    "/usr/lib/sendmail -fgradnin@bb13.betterbox.net -t %s < email.msg", account->new_email );
  system( tmp );
...
void nanny_acnt_request( link_data* link, char* argument )
{
  ...
  sprintf( tmp,
    "/usr/lib/sendmail -fgradnin@bb13.betterbox.net -t %s < email.msg", account->email );  
system( tmp );
...
void do_accounts( char_data* ch, char* argument )
{
  ...
  if( !get_flags( ch, argument, &flags, "spPvclLr", "Accounts" ) )
    return;;

   /* added register code - zemus */
  if( is_set( &flags, 7 ) ) {
    if( ch->pcdata->pfile->account != NULL ) {
      send( ch, "You are already registered with an account.\n\r" );
      return;
      }
    if( *argument == '\0' ) {
      send( ch, "Syntax: account -r <accountname> <password>\n\r" );
      return;
      }
    ...
    account = find_account( arg );
    if( account == NULL ) {
      send( ch, "No such account.\n\r" );
      return;
      }

    if( strcmp( argument, account->pwd ) ) {
      send( ch, "Invalid password.\n\r" );
      return;
      }
    
    ch->pcdata->pfile->account = account;
    send( ch, "You are now registered with account %s.\n\r", account->name );
    return;
    }
 
   /* Added Confirm - Zemus - March 18 */
  if( is_set( &flags, 4 ) ) {
    account = ch->pcdata->pfile->account;
    if( account == NULL ) {
      do_help( ch, "need_acnt" );
      return;
      }
    if( account->email == empty_string ) {
      if( strcmp( argument, account->confirm ) ) {
        do_help( ch, "bad_confirm" );
        return;
        }
      send( ch, "Account Validated!\n\r" ); 
      account->email     = account->new_email;
      account->new_email = empty_string;
      account->confirm   = empty_string;
      save_accounts( );
      ...
      send( ch, "Your account has already been validated.\n\r");
      return;
        }
    }


  /* Added Player Version of Account command - Zemus March 18 */

  if( !has_permission( ch, PERM_SITE_NAMES ) || is_set( &flags, 3 ) ) {
    if( ch->pcdata->pfile->account != NULL ) {
       account = ch->pcdata->pfile->account;
       page_centered( ch, "Account Information" );
       page( ch, "  Account: %s\n\r", account->name );
       page( ch, "    Email: %s\n\r", account->email );
  ...
  /* Account List - Zemus March 19 */
  if( is_set( &flags, 5 ) || is_set( &flags, 6 ) ) {
     page( ch, "%-20s %-10s %s\n\r", "Account", "Validated", "Email" );
     page( ch, "%-20s %-10s %s\n\r", "-------", "---------", "-----" );
     ...
     for( i = 0; i < max_account; i++ ) {
        ...
        if( is_set( &flags, 6 ) ) {
          ...
          for( link = link_list; link != NULL; link = link->next ) {
             if( link->connected != CON_PLAYING )             
               continue;
             if( link->player->pcdata->pfile->account == account ) {
               found = TRUE;
               continue;
               }
             }
        ...
        if( account->email == empty_string ) {
          sprintf( val, "no" );
          sprintf( email, "%s", account->new_email );
          }
        else {
          sprintf( val, "yes" );
          sprintf( email, "%s", account->email );
          }
        if( found )  
           page( ch, "%-20s %-10s %s\n\r", account->name, val, email );
        }         
     return;
    }
...
bool lower_balance( char_data* ch, int amount )
{
  ...
  if( account->balance < amount ) {
    send( ch, "Your account balance is $%.2f - requested purchase is $%.2f.\n\r", account->balance, amount );
    return FALSE;
    } 
