  ...
  if( victim->fighting == ch ) {
    ...
    if( ch->Seen( victim ) )
      send( victim, "%s%s counterattacks you!!%s\n\r",
        bold_v( victim ), ch, c_normal( victim ) );
    ...
    for( int i = 0; i < *victim->array; i++ ) 
      if( ( rch = character( victim->array->list[i] ) ) != NULL 
        ...
        send( rch, "%s%s counterattacks %s!!%s\n\r",
          damage_color( rch, ch, victim ), ch, victim, c_normal( rch ) );
    }
  else {
...
int player_round( char_data* ch, char_data* victim )
{
  ...
  if( wield != NULL ) {
  //following line changed by rengeleif for snorri
    i      = ( shield == NULL && secondary == NULL ? 4 : 3 ); 
  ...
  for( i = 0; i < ( secondary == NULL ? 3 : 4 ); i++ )
    rate += ch->get_skill( attack_skills[i] ); 
...
void trigger_hit( char_data* ch, char_data* victim, obj_data* wield )
{
  ...
  for( oprog = wield->pIndexData->oprog; oprog != NULL;
    ...
    if( oprog->trigger == OPROG_TRIGGER_HIT ) {
      var_ch     = ( ch->Is_Valid( ) ? ch : NULL );
      var_victim = ( victim->Is_Valid( ) ? victim : NULL );
      var_obj    = wield;    
      var_room   = ch->in_room;
      execute( oprog );
      if( !wield->Is_Valid( ) )
...
int damage_armor( char_data* ch, int loc )
{
  ...
  if( has_affect( ch, AFF_ARMOR ) ) {
     absorbed += max( 1, (int) ( affect_level( ch, AFF_ARMOR ) / 10 * 7 ) );
    }
...
/* OLD CODE 
  ...
  if( ch->species == NULL )
    ...
    {
      if( !is_set( &ch->species->act_flags, ACT_HUMANOID ) 
  ...
  send( ch,
	"With a well timed kick you knock %s to the ground!\n\r",
	victim );
  fsend( victim, "%s delivers a quick kick, knocking you to the ground!", ch );
  send( *ch->array, "%s kicks %s, knocking %s to the ground.\n\r",
...
/* NEW CODE */
  ...
  if( has_affect( victim, AFF_FLOAT ) )
    return FALSE;
  ...
  send( ch, "With a well timed kick you knock %s to the ground!\n\r", victim );
  fsend( victim, "%s delivers a quick kick, knocking you to the ground!", ch );
...
int backstab_damage( char_data* ch )
{
  ...
  if( ch->get_skill( SKILL_ASSASSINATE ) == UNLEARNT )
    ch->improve_skill( SKILL_BACKSTAB );
  else
    ch->improve_skill( SKILL_ASSASSINATE );

  i = 10*ch->get_skill( SKILL_BACKSTAB )
    +10*ch->get_skill( SKILL_ASSASSINATE )
    +ch->shdata->level;
  ...
  const char* dt )
{
  ...
  if( has_affect( ch, AFF_CURSE ) ) {
    curse_lvl = affect_level( ch, AFF_CURSE );
    chance -= number_range( (int) ( curse_lvl * 1.5 ), curse_lvl * 4 );
  }
...
void do_engage( char_data* ch, char* argument )
{
  ...
  int          modify;
  int          skill  = ch->get_skill( SKILL_ENGAGE );
  char_data*   victim;
  obj_data*    obj = ch->Wearing( WEAR_HELD_R );
  obj_data*    wield = ch->Wearing( WEAR_HELD_R );
  ...
  if( !can_kill( ch, victim ) ){
    send( ch, "You can't engage them.\n\r" );
    return;
  }

  if( ch->fighting != NULL ) {
    send( ch, "You have already engaged the enemy.\n\r" );
    return;
  } 

  if( skill == 0 ){
    send( ch, "You don't know how to do that.\n\r");
    return;
  }


    send( ch, "You engage %s in combat!!\n\r", victim );
    fsend( *ch->array, "%s engages %s in combat!!", ch, victim );
                                                                 

  if( (ch->Hitroll( wield ) * number_range( 3, 4 )) >(victim->mod_armor* (10-skill)/4)){
    modify = skill * (int)ch->Damroll(wield);  
    attack( ch, victim, "engage" , obj, modify, 0, 0);
  }else{
    send( ch, "Your engaging strike misses the target.\n\r");
  }
