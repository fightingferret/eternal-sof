...
bool target_offensive( char_data* ch, cast_data* cast, char* argument )
{
  ...
  if( victim == ch ) {
    fsend( ch,
      "Your anima speaks up nervously and argues persuasively that casting %s\
 at yourself would be fatuous.", spell_table[ cast->spell ].name );
    return FALSE;
    }
...
{
  if( *argument != '\0' ) {
    fsend( ch, "Casting %s requires no argument as the only possible\
 victim is yourself.", spell_table[ cast->spell ].name );
    return FALSE;
    }
...
bool get_target( char_data* ch, cast_data* cast, char* argument )
{
  ...
  if( type == STYPE_RECALL ) 
	...
	if( matches( argument, town_table[i].name ) )
      ...
      if( ( cast->target = one_character( ch, argument,
