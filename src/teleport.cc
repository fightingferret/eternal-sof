...
bool spell_astral_gate( char_data* ch, char_data*, void* vo, int, int, int )
{
  ...
  int )
{
  ...
  if( victim != NULL ) {
    if( ch != NULL && !consenting( victim, ch, "blinking" ) ) 
      return TRUE;
    ch = victim;
  }
...
bool spell_recall( char_data* ch, char_data* victim, void* vo, int, int, int )
{
  room_data *where = Room( (thing_data*) vo );
  ...
  if( where == NULL ) 
      ...
      if( victim->species != NULL )
	{
	  ...
	  where = get_temple( victim->leader );
	}
      else
	where = get_temple( victim );
    }
...
bool spell_summon( char_data* ch, char_data* victim, void*, int, int, int )
{
  thing_array   list;
  char_data*     rch;
  ...
  if( victim->in_room == ch->in_room || victim->in_room == NULL ) {
    send( ch, "Nothing happens.\n\r" );
    return TRUE;
    }
  ...
  send_seen( victim, "%s slowly fade%s into existence.\n\r",
    &list, list > 1 ? "" : "s" );
  show_room( victim, ch->in_room, FALSE, "-n" );
...
bool spell_transfer( char_data* ch, char_data*, void*, int, int, int )
{
