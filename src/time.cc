...
const char* timezones [24] = { "gmt-11", "gmt-10", "gmt-9", "gmt-8",
  "gmt-7", "pacific",
  "mountain",  "central", "eastern", "atlantic", "gmt-1",
  "greenwich",
  "gmt+1", "gmt+2", "gmt+3",
  "gmt+4", "gmt+5", "gmt+6", "gmt+7", 
  "gmt+8", "japan", "sydney", "gmt+11", "gmt+12" };
...
void do_system( char_data* ch, char* )
{
  ...
  i = 100.*total_time[ TIME_ACTIVE ].hundred( )/i;
...
void do_time( char_data* ch, char* argument )
{
  ...
  if( !get_flags( ch, argument, &flags, "zc", "Time" ) )
    return;
  ...
  if( flags == 0 ) {
    send( ch, "The current MUD time is %02d:%02d on the %d%s day of %s.\n\r",
weather.hour,
          weather.minute, weather.day,
number_suffix( weather.day ), month_name[weather.month] );
    send( ch, "----------------------------------------\n\r" );
    send( ch, "Your timezone is set to %s.\n\r", timezones[pc->timezone] );
    sprintf_time( tmp, current_time, pc->timezone );
    send( ch, "Your current time is %s.\n\r", tmp );
    return;
    }

  if( is_set( &flags, 1 ) ) {
     if( is_set( ch->pcdata->pfile->flags, PLR_AUTOPROMPT ) ) {
       remove_bit( ch->pcdata->pfile->flags, PLR_AUTOPROMPT );
       send( ch, "Autoprompt mode off.\n\r" );
       return;
       }
     set_bit( ch->pcdata->pfile->flags, PLR_AUTOPROMPT );
     send( ch, "Autoprompt mode on.\n\r" );
     return;  
    }
...
int time_arg( char*& argument, char_data* ch )
{
  ...
  send( ch, "Unknown unit of time.\n\rKnown units are seconds, minutes, hours, days, and years.\n\r" ); 


  return -1;
}

/* uptime command added zemus sept 14 */
void do_uptime( char_data* ch, char* )
{
  char         tmp [ MAX_STRING_LENGTH ];

  sprintf_minutes( tmp, current_time-boot_time );
  send( ch, "The system has been running for %s.\n\r", tmp );

  return;
}

/* settime command added by zemus sept 14 */
void do_settime( char_data* ch, char* argument )
{
 int      hour;
 
 if( !has_permission( ch, PERM_ADMIN ) ) {
   send( ch, "You do not have permission to use this command.\n\r" );
   return;
   }
 
 if( argument == NULL || !strcasecmp( argument, "" ) ) {
   send( ch, "syntax: settime <hour>\n\r" );
   return;
   }

 hour = atoi( argument );
 if( hour < 0 || hour > 24 ) {
   send( ch, "The hour must be between 0 and 24.\n\r" );
   return; 
   }

 weather.hour = hour;
 send( ch, "The hour has been set to %d.\n\r", hour );  
