...
bool all_programs_disabled = FALSE;
...
void* proc_arg( arg_type* arg )  
{
  ...
  if( arg->family == variable ) {
    pntr = (void**) arg->value;
    // Zeth, 12/12/99 The following comment was here BEFORE.
    //Changed == to =
    // I changed it back ie: I changed the = to ==
    // 1) Don't fix it if it ain't broken
    // 2) It's broken now, so I'm fixing it.
    // 3) Problem: *pntr == NULL does not make sense. This may
    //      return 1 for a void *, which is clearly not what
    //      was intended. However, I make the change anyways.
    // Ok, based on TFH code, I changed *pntr == NULL to NULL.
    // it makes more sense now.
    // return  ( arg->neg ? NULL : *pntr );
    /* PUIOK 29/1/2000 -- PUT it back the way it was, it made
       and now makes perfect sense.  Don't fix it if it ain't
       broken, else you'll generate more bugs.  The above change
       stops NOT from working properly. */
    return( arg->neg ? (void*) ( *pntr == NULL ) : *pntr );
  }
...
// Zeth Dec. 18, 1999
// I commented the following and replaced it with the stuff after it
// using TFH code as a model.
/*    for( i = 0; i < var_room->contents; i++ ) {
  ...
  /* PUIOK 27/2/2000 -= and += fix */
  if( afunc->func->func_call == code_set_equal
    || afunc->func->func_call == code_plus_equal
    || afunc->func->func_call == code_minus_equal ) {
    farg[0] = afunc->arg[0];         
