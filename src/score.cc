...
void do_identity( char_data* ch, char* )
{
  ...
  if( pc != NULL ) {
    ...
    page( ch, "        Deaths: %-7d Recog Char: %-12d PKills: %d\n\r",
      ch->shdata->deaths,
      pcdata->recognize == NULL ? 0 : pcdata->recognize->size,
      pcdata->pfile->pkcount );
  ...
  if( pc != NULL ) {
    if( ch->shdata->level >= LEVEL_APPRENTICE ) 
      sprintf( buf+25, "    Trust: %d\n\r", ch->pcdata == NULL
        ? LEVEL_APPRENTICE - 2 : ( ch->pcdata->trust == 0
        ? ch->shdata->level : ch->pcdata->trust ) );
