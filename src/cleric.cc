...
void heal_victim( char_data* ch, char_data* victim, int hp/*, bool msg*/ )
{
...
bool spell_group_critical( char_data* ch, char_data* victim, void*,
  int level, int, int )
{
  char_data* gch;
  ...
  int level, int, int )
{
  ...
  for( int i = victim->wearing.size-1; i >= 0; i-- ) {
    ...
    send( victim, "%s which you are wearing turns to dust.\n\r",
      obj->Name( ch, obj->number ) );
    send_seen( victim, "%s which %s is wearing turns to dust.\n\r",
      obj->Name( ch, obj->number ), victim );
    obj->Extract( );
  }
...
bool spell_silence( char_data* ch, char_data* victim, void*, int level,
  int duration, int )
{
...
bool spell_create_water( char_data* ch, char_data* victim, void* vo,
  int level, int duration, int type )
{
  obj_data*         obj  = (obj_data*) vo;
  content_array*  where;
  int            liquid;
  ...
  if( type == STYPE_LIQUID_THROW || type == STYPE_DRINK
    || type == STYPE_FILL )
    return FALSE;
...
bool spell_purify( char_data* ch, char_data* victim, void*, int, int, int )
{
...
bool spell_sanctify( char_data* ch, char_data*, void *vo, int level,
  int, int )
{
  obj_data *obj = (obj_data*) vo;
  int dam;
...
bool spell_animate_clay( char_data* ch, char_data*, void*, int, int, int )
{
  char_data*       golem;
  ...
  for( int i = 0; i < ch->followers; i++ ) { 
    ...
    if( is_set( &golem->status, STAT_PET ) 
      ...
      send( ch,
        "The incarnation fails as you can only control one golem at a time.\n\r" );
      return TRUE;
      }
    }
...
bool spell_resurrect( char_data*, char_data*, void*, int, int, int )
{
  return TRUE;
}
...
bool spell_fear( char_data* ch, char_data* victim, void*, int level,
  int, int type )
{
  ...
  if( ch != NULL )
    ...
    send( victim, "You cringe in fear!\n\r" );
  send( *victim->array, "%s cringes in fear!\n\r", victim );
