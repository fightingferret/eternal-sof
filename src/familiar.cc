...
void do_switch( char_data* ch, char* argument )
{
  ...
  else 
  {
    ...
    if( !is_mob( victim ) && victim->link != ch->link ) 
    {
	  send( ch, "You cannot switch to players.\n\r" );
	  return;
    }
    
    if( victim->link != NULL && victim->link != ch->link )
    {
    ...
    if( victim->pcdata != NULL
      && is_set( victim->pcdata->pfile->flags, PLR_FREEZE ) )
    {
	  send( ch, "%s is frozen, best not switch.\n\r", victim );
	  return;
    }
  }
  ...
  /*pc->link     = NULL; ** COMMENTED PUIOK 28/12/1991 FOR SWITCH_MESSAGING */
  victim->link     = link;
