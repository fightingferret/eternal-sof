...
/* ADDED -- PUIOK 8/1/2000 */

#define IDENTERR_NET "Network Error"
#define IDENTERR_GEN "General Error"
#define IDENTERR_IDT "ERROR"

#define REPORTER  (*reporter)(int, char *, char *, char *, link_data *)

void   id_create_request        (link_data *, int, struct sockaddr_in *, void REPORTER);
void   id_check_request         (link_data *);
void   id_kill_request          (link_data *);
...
class Link_Data
{
 public:
  ...
  text_data*         paged;
  char*        rec_pending;       //any pending stuff that has been recieved
  char*       send_pending;       //any pending stuff to be sent
  char*           rec_prev;       //the previous command used by link
  bool             command;       //is there a command to be executed?
  
  /* PUIOK 7/1/2000 -- IDENT */
  struct ident_set   ident;
