...
int         exits_prompt       ( char*, char_data*, room_data* = NULL );
void        read_exits         ( FILE*, room_data*, int );
void        autoexit           ( char_data*, room_data* = NULL );
...
#define RFLAG_STAT_CORRECTED       29
#define RFLAG_BOUNTY_OFFICE        30
#define MAX_RFLAG 31
...
class Room_Data : public Thing_Data
{
 public:
  ...
  virtual int Type    ( )        { return ROOM_DATA; }
          int Light   ( int );
          int Light   ( int, bool&, bool& );
  
  const char*  Location   ( content_array* = NULL );
  bool         is_dark    ( char_data* = NULL );
  bool         Seen       ( char_data* );
};
...
#define SECT_UNKEPT_ROAD           17
#define SECT_TRAIL                 18
#define SECT_PATH                  19
#define SECT_SWAMP                 20
#define SECT_JUNGLE                21
#define SECT_ARCTIC                22
#define SECT_STREAM                23
#define MAX_TERRAIN                24
...
extern const terrain_type terrain  [ MAX_TERRAIN ];
