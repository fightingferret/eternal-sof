...
void do_ban( char_data* ch, char* argument )
{
  ...
  if( !get_flags( ch, argument, &flags, "srnp", "Ban" ) )
    return;

  /* Individual Player Banning - Zemus March 11 */
  if( is_set( &flags, 3 ) ) {
  ...
  if( ( account = account_arg( argument ) ) == NULL ) {
    ...
    if( ( account = pfile->account ) == NULL ) {
      send( ch, "That player doesn't have an account which makes banning it difficult.\n\r" );
      return;
      }
    }
  ...
  if( ch->pcdata->pfile->account == account ) {
    send( ch, "You cannot ban your own account.\n\r" );
    return;
    }
