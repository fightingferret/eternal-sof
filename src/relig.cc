...
void do_sacrifice( char_data* ch, char* argument )
{
  ...
  if( is_set( obj->pIndexData->extra_flags, OFLAG_NOSACRIFICE ) ) {
    ...
    sprintf(bug_string, "%s has sacrificed a no sac item to %s.",
     ch->real_name( ), religion_table[ ch->pcdata->religion ].name);
    bug(bug_string);
    return;
    }
...
/* Action Check changed by Zemus to use a value of zero to run the action
  ...
  if(ch->pcdata->religion != i){
    sprintf(bug_string, "%s has changed religion to %s.", 
     ch->real_name( ), religion_table[ i ].name); 
    bug(bug_string);
  }
  ...
  ch->pcdata->religion = i;
    ...
    sprintf(bug_string, "%s has sacrificed %s to %s.", ch->real_name( ),
     obj->Seen_Name( NULL ), religion_table[ i ].name); 
    bug(bug_string);
