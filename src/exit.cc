...
const char* Exit_Data :: Keywords( char_data* ch )
{
  ...
  sprintf( tmp, "%s 1 %s %s", keywords, dir_table[ direction ].name,
           Seen_Name( ch ) );
 ...
 if( ch->shdata->level < LEVEL_APPRENTICE ) {
    ...
    if( exit != NULL ) {
      ...
      if( !is_set( &exit->exit_info, EX_CLOSED ) )
        ...
        send( ch, "%-5s - Closed %s - To Room %s [%d]\n\r", dir_table[
           exit->direction ].name, exit->name, exit->to_room->name,
           exit->to_room->vnum );
      } 
    }
  }
...
/* MODIFIED -- PUIOK 22/1/2000 'room_data* room )' */

int exits_prompt( char* tmp, char_data* ch, room_data* in_room )
{
  exit_data*  exit;
  char*     string  = tmp;
  bool       water  = FALSE;
  ...
  for( int i = 0; i < in_room->exits; i++ ) {
    exit = in_room->exits[i];
...
void autoexit( char_data* ch, room_data* in_room )
{ 
  ...
  *buf = '\0';
    ...
    if( !is_set( &exit->exit_info, EX_CLOSED )
      ...
      if( water_logged( room ) ) 
        sprintf( buf+strlen( buf ), " %s%s%s", blue( ch ),
          dir_table[ exit->direction ].name, c_normal( ch ) );
      else
  ...
  if( *buf == '\0' ) {
    send_color( ch, COLOR_EXITS, "[Exits: none]\n\r" );
    }
  else {
    strcat( buf, "]\n\r" );
    send_color( ch, COLOR_EXITS, buf );
    }
}
...
/*  for( int i = obj->contents.size - 1; i >= 0; i-- ) {
    if( ( trap = object( obj->contents[i] ) ) == NULL )
      continue;
    if( 
...
/*  for ( i = obj->contents.size - 1; i >= 0; i-- ) {
     ...
     if ( trap->pIndexData->item_type == ITEM_TRAP && trap->Seen( ch ) ) {
        for( oprog = trap->pIndexData->oprog; oprog != NULL; 
             oprog = oprog->next ) {
...
obj_data* has_key( char_data* ch, int vnum )
{
  ...
  if( (obj = ch->Wearing( WEAR_WAIST,LAYER_UNDER )) != NULL ){
...
void do_lock( char_data* ch, char *argument )
{
  ...
  if( ( thing = one_thing( ch, argument, "lock",
    (thing_array*) &ch->in_room->exits, ch->array, &ch->contents ) ) == NULL )
    return;
  ...
  if( ( obj = object( thing ) ) != NULL ) {
    ...
    for( oprog = obj->pIndexData->oprog; oprog != NULL; oprog = oprog->next )
      if( oprog->trigger == OPROG_TRIGGER_LOCK ) {
        ...
        var_container = obj;
        execute( oprog );
        if( !obj->Is_Valid( ) || !ch->Is_Valid( ) || !key->Is_Valid( ) )
          return;
        break;
      }
...
{  
  ...
  act( ch, prog_msg( action, lock_door_msg[0],
    lock_door_msg[1] ), exit, key );
  act_notchar( prog_msg( action, lock_door_msg[2],
    lock_door_msg[3] ), ch, exit, key );
  ...
  if( ( exit = reverse( exit ) ) != NULL ) {
    set_bit( &exit->exit_info, EX_LOCKED );
    act_room( room, prog_msg( action, lock_door_msg[4],
      lock_door_msg[5] ), exit );
    }
...
{ 
  ...
  if( ( exit = reverse( exit ) ) != NULL ) {
    remove_bit( &exit->exit_info, EX_LOCKED );
    act_room( room, prog_msg( action, unlock_door_msg[4], unlock_door_msg[5] ),
              exit );
    }
...
void do_pick( char_data* ch, char* argument )
{
  ...
  obj_data* lockpick = one_object( ch, "small lock pick", empty_string,
  ...
  if( ( obj = object( thing ) ) != NULL ) {
    ...
    if ( IS_NPC( ch ) || ch->shdata->skill[ SKILL_PICK_LOCK ] == UNLEARNT ) {
    ...
    if( is_set( &obj->value[1], CONT_PICKPROOF ) ) {
      send( ch, "You fail to pick the lock and are fairly sure\
...
bool pick_door( char_data* ch, exit_data* exit )
{
  ...
  if( is_set( &exit->exit_info, EX_PICKPROOF ) ) {
    send( ch, 
"You fail to pick the lock and are fairly sure it is impossible to pick.\n\r" ); 
    return FALSE;
  }
...
/* PUIOK 4/6/2000 -- moved to room.cc -- PURGE if more then 2 weeks old
void do_rnew( char_data* ch, char* argument )
{
  room_data*  room;
  int         vnum  = 1;
  ...
  {
    vnum = atoi( argument );
    room = get_room_index( vnum, FALSE );
    if ( room != NULL )
    {
      send( ch, "A room with that vnum already exists!\n\r");
      return;
    }
  }
  ...
  send( ch, "Room created with vnum %d\n\r", vnum );
}
*/
...
/* added BOTH flag -- Zemus March 21 */
...
{ 
  ...
  if( !get_flags( ch, argument, &flags, "1b", "dset" ) )
    return;
  ...
  if( !can_edit( ch, exit->to_room ) )
    send( ch, "You don't have permission to edit the return exit.\n\r" );
  ...
  class type_field type_list2[] = {
    { "size",  MAX_SIZE,  &size_name[0],  &size_name[1],  &back->size },
    { "",      0,         NULL,           NULL,           NULL        }
    };

  if( process( type_list, ch, dir_table[ exit->direction ].name, argument ) ) {
    if( both == TRUE ) {
      send( ch, "Opposite Exit " );
      if( process( type_list2, ch, dir_table[ back->direction ].name, argument ) )
        return;
      }
    return;
    }
  ...
  class string_field string_list2[] = {
    { "name",      MEM_EXIT,  &back->name,       NULL },
    { "keywords",  MEM_EXIT,  &back->keywords,   NULL },
    { "",          0,         NULL,              NULL },   
    };
  ...
  if( process( string_list, ch, dir_table[ exit->direction ].name, argument ) ) {
    if( both == TRUE ) {
      send( ch, "Opposite Exit " );
      if( process( string_list2, ch, dir_table[ back->direction ].name, argument ) )
        return;
      }
    return;
    }
  ...
  class int_field int_list2[] = {
    { "strength",          0,   10,  &back->strength  },
    { "light",             0,  100,  &back->light     },
    { "",                  0,    0,  NULL             }
    };
  ...
  if( process( int_list, ch, dir_table[ exit->direction ].name, argument ) ) {
    if( both == TRUE ) {
      send( ch, "Opposite Exit " );
      if( process( int_list2, ch, dir_table[ back->direction ].name, argument ) )
        return;
      }
    return;
    }
...
bool spell_wizard_lock( char_data*, char_data*, void*, int, int, int )
{
  return TRUE;
}
