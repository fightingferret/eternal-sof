...
*/
 ...
 /* New PK Alignment Check - Zemus July 14 */
/*
 bool       cankill = FALSE;
 bool    needbounty = FALSE;


 switch( ch->shdata->alignment ) {
  
  case ALIGN_LAWFUL_GOOD:
    if( victim->shdata->alignment != ALIGN_LAWFUL_NEUTRAL &&
        victim->shdata->alignment != ALIGN_LAWFUL_EVIL &&
        victim->shdata->alignment != ALIGN_PURE_NEUTRAL && 
        victim->shdata->alignment != ALIGN_NEUTRAL_EVIL &&
        victim->shdata->alignment != ALIGN_CHAOTIC_EVIL &&
        victim->shdata->alignment != ALIGN_CHAOTIC_NEUTRAL ) {
      cankill    = FALSE;
      needbounty = TRUE;
      }
    else {
  ...
  case ALIGN_LAWFUL_NEUTRAL:
    cankill    = TRUE;
    needbounty = TRUE;   
    break;

  case ALIGN_LAWFUL_EVIL:
    if( victim->shdata->alignment != ALIGN_LAWFUL_GOOD &&
        victim->shdata->alignment != ALIGN_LAWFUL_NEUTRAL &&
        victim->shdata->alignment != ALIGN_NEUTRAL_GOOD &&
        victim->shdata->alignment != ALIGN_PURE_NEUTRAL &&
        victim->shdata->alignment != ALIGN_CHAOTIC_GOOD &&
        victim->shdata->alignment != ALIGN_CHAOTIC_NEUTRAL ) {
      cankill    = FALSE;
      needbounty = TRUE;
      }
  ...
  case ALIGN_PURE_NEUTRAL:
    cankill    = TRUE;
    needbounty = FALSE;
    break;

  case ALIGN_NEUTRAL_EVIL:
    if( victim->shdata->alignment != ALIGN_LAWFUL_NEUTRAL &&
        victim->shdata->alignment != ALIGN_LAWFUL_GOOD &&
        victim->shdata->alignment != ALIGN_PURE_NEUTRAL && 
        victim->shdata->alignment != ALIGN_NEUTRAL_GOOD &&
        victim->shdata->alignment != ALIGN_CHAOTIC_GOOD &&
        victim->shdata->alignment != ALIGN_CHAOTIC_NEUTRAL ) {
  ...
  case ALIGN_CHAOTIC_NEUTRAL:
    cankill    = TRUE;
    needbounty = FALSE;
    break;

  case ALIGN_CHAOTIC_EVIL:
    cankill    = TRUE;
    needbounty = FALSE;
    break;
  }
 ...
 if( cankill && needbounty && victim->pcdata != NULL ) {
   if( victim->pcdata->pfile->bounty == 0 ) {
     if( msg ) {
       send( ch, "You would never kill without a bounty.\n\r" );
       }
     return FALSE;
     }
  }
 ...
 if( !cankill ) {
   if( msg ) {
     send( ch, "You would never kill a %s aligned person.\n\r", alignment_table[ victim->shdata->alignment ].name );
     }
   return FALSE;
   }
*/
...
bool can_kill( char_data* ch, char_data* victim, bool msg )
{
  ...
  {
    send( ch, "You cannot attack anything in vapor mode.\n\r" );
    return FALSE;
  }
  
  if( is_set( victim->affected_by, AFF_SANCTUARY )
    || is_set( &victim->status, STAT_VAPOR ) ) {
