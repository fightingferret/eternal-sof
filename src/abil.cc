  ...
  for( j = 0; j < MAX_SKILL_CAT; j++ ) {
    if( !strncasecmp ( skill_cat_name[j], arg, length ) ) {
      for( i = 0; i < MAX_SKILL; i++ ) {
        ...
        if( skill_table[i].level[clss] >= LEVEL_APPRENTICE ) {
  ...
  *buf = '\0';
  ...
  for( i = 0; i < MAX_SKILL; i++ ) {
    ...
    if( ch->shdata->skill[i] >= 7 ) {
      fsend( ch, "You know %s as well as you can be taught it.  It can only\
 be improved by practice now.\n\r", skill_table[i].name );
      return;
      }   
...
void char_data :: improve_skill( int i )
{
  ...
  */
   ...
   * than both.  
   * Random between 0 and 200 must be greater than shdata->level makes
   * it harder to get pracs/improves at higher levels
   */ 
  ...
  if( number_range( 0, 30+j ) < Intelligence( )
    ...
    if( shdata->skill[i] == 10 || number_range( 0, 2 ) != 0 ) {
      pcdata->practice++;
      send_color( this, COLOR_IMPROVES,
        "\n\r** Your use of %s gives you a practice point. **\n\r\n\r",
        skill_table[i].name );
      }
    else {
      /* otherwise improve. */
