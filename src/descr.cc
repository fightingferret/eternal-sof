...
void write_extras( FILE* fp, extra_array& list )
{
  for( int i = 0; i < list; i++ ) {
    if(list[i] == NULL)
    {
      roach("Write extra: Invalid extra entry skipped to avoid crash");
      continue;
    } else {
      fflush(fp);
      if( list[i]->keyword != NULL) ;  /* If crash, crash here to lessen corruption*/
    }
...
void copy_extras(extra_array& to, extra_array& from)
{
  extra_data*   extra;
  ...
  for( int i = 0; i < from; i++ ) {
    ...
    extra->keyword  = alloc_string( from[i]->keyword, MEM_EXTRA );
    extra->text     = alloc_string( from[i]->text, MEM_EXTRA );
...
/*
 *    SEND EXTRA  16/5/2000
 */


bool send_extra( char_data* ch, extra_array* array, char* keyword )
{
  char          tmp [ 3*MAX_STRING_LENGTH ];
  char*      desc;

  if( ( desc = one_extra_desc( array, keyword ) ) != NULL )
  {
