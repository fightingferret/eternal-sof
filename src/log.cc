  ...
  else
  {
    ...
    if( pfile != NULL )
    {
      if( !has_permission( ch, PERM_PLAYERS )
        || ( victim != NULL && victim->pcdata->trust >= ch->pcdata->trust )
        || pfile->trust >= ch->pcdata->trust )
      {
        send( ch, "You lack the permission or level to look at that log.\n\r" );
        return;
      }
...
void do_olog( char_data* ch, char* argument )
{
  ...
  if( *argument == '\0' )
    ...
    if(  obj_clss == NULL )
      {
      send( ch, "You are not editting an object so must specify a vnum.\n\r" );
      return;
      }
    i = obj_clss->vnum;
