...
void   remove_from      ( char_data*, thing_array* , char_data* );

const char* wear_name [] = { "take", "finger", "neck", "body", "head",
  "legs", "feet", "hands", "arms", "light.pos", "unused1", "waist", "wrist",
...
const int wear_index [ MAX_WEAR ] = { ITEM_TAKE, ITEM_WEAR_FINGER,
  ITEM_WEAR_FINGER, ITEM_WEAR_NECK, ITEM_WEAR_LIGHT, ITEM_WEAR_BODY,
...
const char* reset_wear_name[] = { "ground", "inventory",
  ...
  "neck", "light.pos", "body", "head", "legs", "feet", "hands", "arms",
...
bool cant_wield( char_data* ch, obj_data* obj, int slot, int layer )
{
  obj_data*    object;
  
  if( slot != WEAR_HELD_L && slot != WEAR_HELD_R )
    return FALSE;
  
  if( layer != LAYER_BASE )
    return TRUE;
  
  if( is_set( obj->pIndexData->extra_flags, OFLAG_TWO_HAND ) )
  {
    if( ch->Wearing( WEAR_HELD_L ) || ch->Wearing( WEAR_HELD_R ) )
      return TRUE;
  }
  else
    for( int i = 0; i < ch->wearing; i++ ) {
      object = (obj_data*) ch->wearing[i];
      if( object->position == WEAR_HELD_R || object->position == WEAR_HELD_L )
        if( is_set( object->pIndexData->extra_flags, OFLAG_TWO_HAND ) )
          return TRUE;
    }
...
*/
  ...
  if( ch->shdata->level < LEVEL_APPRENTICE && ( ch->shdata->race <
    MAX_PLYR_RACE
...
/* added thing for wear trigger - zemus aug 20 */
thing_data* weartrigger( thing_data* t1, char_data* ch, thing_data* )
{
  obj_data*      obj = (obj_data*) t1;
  oprog_data*     oprog;

   /* added code for special wear triggers */
...
/* Wear order check removed - commented out - PUIOK 25 July 2000 */
...
/* I moved the wear trigger out of equip() and into its own thing_Data
function... i havn't seen any side affects of this and wear triggers
appear to be working.  -- zemus aug 20 */

 void do_wear( char_data* ch, char* argument )
{
  thing_array*     array;
  thing_array wear_array;
  ...
  thing_func*     func  [ 9 ]  = { cantwear, stolen, bladed,
                                   dishonorable, levellimit,
                                   antiobj, needremove, weartrigger, NULL };

  sort_objects( ch, *array, (thing_data*) &wear_array, 9, subset, func );
...
void equip( char_data* ch, obj_data* obj )
{
  ...
  if( is_set( obj->extra_flags, OFLAG_NOREMOVE ) )
    send( ch, "%s is cursed and welds itself to your body!!\n\r", obj );
...
thing_data* nocatch( thing_data* thing, char_data*, thing_data* )
{
  ...
  // return( obj->position == WEAR_FLOATING ? (thing_data*)NULL : obj );
  // of course ya can catch it...
  return obj;
}
...
void do_remove( char_data* ch, char* argument )
{
  ...
  if( contains_word( argument, "from", arg ) ) {
    ...
    if( ( array = several_things( ch, arg, "remove",
      &victim->wearing ) ) != NULL )
      remove_from( ch, array, victim );
    return;
    }
...
/* PUIOK 5/6/2000 -- remove from mount */
void remove_from( char_data* ch, thing_array* array, char_data* victim )
{
  char             buf  [ TWO_LINES ];
  obj_data*        obj;
  thing_array   subset  [ 5 ];
  thing_func*     func  [ 5 ]  = { noremove, nocatch, heavy,
                                   many, NULL };
  ...
  sprintf( buf, "from %s", victim->Name( ch ) );

  page_priv( ch, NULL, empty_string );
  page_priv( ch, &subset[0], "you can't remove", NULL, buf );
  page_priv( ch, &subset[1], "can't catch" );
  page_priv( ch, &subset[2], "can't carry" );
  page_priv( ch, &subset[3], "can't handle" );
  page_publ( ch, &subset[4], "remove", NULL, buf );/**/
  
  for( int i = 0; i < subset[4]; i++ )
    if( ( obj = object( subset[4].list[i] ) ) != NULL ) {
