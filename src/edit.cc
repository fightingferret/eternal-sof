  ...
  for( ; *input != '\0'; ) {
    if( isspace( *input ) ) {
      if( !was_space ) {
        ...
        last_counts = 0;
        if( was_period ) {
          for( letter = input+1; *letter != '\0'; letter++ )
            if( !isspace( *letter ) ) {
              if( *letter == '@' ) {
    ...
    if( output - line > 73 + no_counts ) {
      ...
      else if( ( line = output-1 ) > word && *(word+1) != ' ' ) {
        do {
          *(line+1) = *line;
          } while( --line > word );
        output++;
      ...
      *(word+1) = '\r';
      line = word+2;
      word = NULL;
      no_counts = last_counts;
      last_counts = 0;
      }

    if( isperiod( *input ) && was_period ) {
        word = output+1;
        last_counts = 0;
      }

    was_period = isperiod( *output++ = *input++ );
    was_space = FALSE;
    }

  for( ; output > start && isspace( *(output-1) ); output-- ) ;
  *output = '\0';
    ...
    *word = '\0';
    ...
    if( *argument == '-' && i != -1 ) {
      ...
      if( strlen( pString ) > 2*MAX_STRING_LENGTH && *argument != '\0' ) {
        send( "Due to internal limits files can be no longer then 6k characters.\n\rPushing this will crash the mud.\n\r", ch );
