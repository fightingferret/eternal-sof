...
help_data* find_help( char_data* ch, const char* argument )
{
  ...
  for( ; pos < max_help; pos++ ) {
    ...
    if( can_read( ch, help ) ) {
      if( first != -2 ) {
        if( first != -1 ) {
          page( ch, "More than one match was found - please be more specific\
 in what topic you\n\rwant help on.\n\r\n\r" );
          page( ch, "  [%3d] %s:%15s%s\n\r", first,
...
void do_help( char_data* ch, char* argument )
{
  ...
  if( help->immortal != empty_string && is_immortal( ch ) ) {
    page( ch, "\n\r" );
...
void do_index( char_data* ch, char* argument )
{
  ...
  if( *argument =='\0' ) {
    ...
    page( ch, "\n\r%s", j%4 != 0 ? "\n\r" : "" );
    page_centered( ch, "[ Type index <category> to see a list of help files\
 in that category. ]" );
    return;
    }
...
/* done added zemus - april 16 */

  if( !strncasecmp( argument, "done", 4 ) ) {
    if( help != NULL ) {
      send( ch, "You stop editing %s\n\r", help->name ); 
      ch->pcdata->help_edit = NULL;
      return;
      }
    else {
      send( ch, "You are not editing a help file!\n\r" );
      return;
      }
    }
  ...
  if( !strncasecmp( argument, "delete", 6 ) ) {
    ...
    sprintf( buf, "Help file deleted: %s (%s)", help->name, ch->descr->name );
    info( "", LEVEL_APPRENTICE, buf, IFLAG_WRITES ); 
    delete help;
    return; 
  }
  
  if( !strncasecmp( argument, "-c new ", 7 ) )
  {
    argument += 3;
    if( help != NULL )
      copy_help = help;
    ...
    {
      send( ch, "Use hedit to select the help file to copy from.\n\r" );
      return;
    }
  }
  
  if( copy_help != NULL || !strncasecmp( argument, "new ", 4 ) )
  {
    argument += 4;
    if( argument[0] == '\0' )
    {
    ...
    {
      if( copy_help->text != empty_string )
        help->text = alloc_string( copy_help->text, MEM_HELP );
      else
        copy_help->text = (char*) empty_string;
    }
    ...
    if( copy_help != NULL )
      send( ch, "Help subject created and contents copied from '%s'.\n\r",
        copy_help->name );
    else
  ...
  if( !strcmp( help->name, "motd" ) )
    motd_change = current_time;
...
void help_stat( char_data *ch, help_data* help )
{
  char    tmp  [ FOUR_LINES ];
...
#undef hcn
  ...
  if( matches( argument, "name" ) ) {
    ...
    if( ( pos = pntr_search( help_list, max_help, argument ) ) < 0 )
      pos = -pos-1;
    insert( help_list, max_help, help, pos );
    return;
    }
