...
bool char_data :: Recognizes( char_data *victim )
{
  ...
  if( link != NULL && link->player != this && link->player == victim )
    return TRUE;
...
void do_introduce( char_data *ch, char *argument )
{
  ...
  if( skill_language( victim, ch->pcdata->speaking ) == 0 ) {
    fsend( ch, "You attempt to introduce yourself to %s, but %s does not seem to understand a word you are saying.\n\r",
...
{
 /* level check changed to PERM_PLAYERS - zemus jan 14 */

  if( ch == victim || has_permission( ch, PERM_PLAYERS ) )
    return TRUE;
  ...
  if( word != empty_string ) {
    fsend( ch, "Since %s does not automatically consent to your actions the %s fails.", victim, word );
