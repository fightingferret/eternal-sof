...
{  
  ...
  /* Check for site ban - Zemus - Feb 27 */
  if( is_banned( link->host ) )
    {
     help_link( link, "site_banned" );
     sprintf( tmp, "Connection from %s [BANNED]", link->host );
     info( "", LEVEL_ENTITY, tmp, IFLAG_LOGINS, 3, NULL, NULL, PERM_SITE_NAMES );
     fp = open_file( FILE_DIR, "iplog.txt", "a" );
     if( fp == NULL ) {
       roach( "Could not open ip log file." );
       return;
       }
     fprintf( fp, "[BANNED] %s at %s", link->host,  (char*) ctime( &current_time
) );
     fclose( fp );
     close_socket( link );
     return;
