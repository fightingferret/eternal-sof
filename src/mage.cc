...
bool spell_meteor_swarm( char_data* ch, char_data*, void*, int level,
  int, int )
{
  char_data* rch;
...
bool spell_maelstrom( char_data* ch, char_data*, void*, int level,
  int, int )
{
  char_data*   rch;
...
void sleep_affect( char_data* ch, char_data* victim, int level, int duration )
{
  ...
  if( is_set( victim->affected_by, AFF_SLEEP_RESIST ) ){
    if( victim->position > POS_SLEEPING ){
      ...
      send( ch, "%s shrugs off your feeble spell. \n\r", victim );
    }else
      send( ch, "%S is already sleeping. \n\r", victim );
    return; 
  }
  ...
  int level, int, int )
{
  ...
  if( obj->value[0] < 0 || obj->value[0] >= 3 ) {
    fsend( *ch->array,
      "%s starts to glow, but the light turns black and then fades.", obj );
    return TRUE;
    }
...
bool spell_replicate( char_data* ch, char_data*, void* vo, int, int, int )
{
  obj_data*        obj  = (obj_data*) vo; 
...
bool spell_identify( char_data* ch, char_data*, void* vo, int, int, int )
{
  ...
  roach( "Identify -> Obj = %s", obj );

  obj = sunder( obj );

  roach( "Identify -> sunder succeeded!" );
...
bool spell_obscure( char_data* ch, char_data*, void* vo, int, int, int )
{
  ...
  int level, int duration, int )
{
  ...
  for( int i = 0; i < *ch->array; i++ ) {
    ...
    if( rch == NULL || rch == ch || !rch->Can_See( )
      || ( rch->species != NULL
    ...
    if( makes_save( rch, ch, RES_MAGIC, SPELL_BLINDING_LIGHT, level ) ) {
      send( rch,
        "Fortunately you were not looking at the blast and are unaffected.\n\r" );
      }
...
bool spell_eagle_eye( char_data* ch, char_data* victim, void*, int level,
  int, int )
{
  room_data* room;
...
bool spell_hawks_view( char_data* ch, char_data* victim, void*, int level,
  int, int )
{
  room_data* room;
