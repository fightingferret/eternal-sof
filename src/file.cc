...
FILE* open_file( const char* path, const char* type, bool fatal )
{
  ...
  /* If file not opened, fatal error */
  /* Zerli 14-10-2000 */
  fatal = TRUE;
  roach( "Unable to open file = '%s'", path );

  sprintf_minutes( tmp, current_time-boot_time );
  bug( tmp );

  if ( errno = EMFILE )
    roach( "Maximum number of file handles for process reached!!" );
...
void write_all( bool forced )
{
  ...
  save_ddatabase( );  /* 16/4/2000 PUIOK */
...
void do_write( char_data* ch, char* argument )
{
  ...
  if( !strcasecmp( argument, "quests" ) ) {
    if( !has_permission( ch, PERM_QUESTS ) ) {
  ...
  /* 16/4/2000 PUIOK */
  if( fmatches( argument, "ddatabase" ) ) {
    if( !has_permission( ch, PERM_BASIC, TRUE ) )
      return;
    save_ddatabase( );
    send( ch, "Ok.\n\r" );
    sprintf( buf, "Description Database written (%s).", ch->real_name() );
    info( "", LEVEL_APPRENTICE,  buf, IFLAG_WRITES, 1, ch );
    return;
    }
