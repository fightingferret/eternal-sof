...
const char* empty_msg = "You beg and plead to Creslin, God-King of The Whole\
 World,  but only attract a following of off duty mail daemons\
 who taunt you that there is nothing to return you.";

const char* junk_undo_msg = "You abase yourself before Creslin, God-King\
 of The Whole World for return your lost items.  Fortunately\
 it seems he noticed your mortal pleading.";
...
thing_data* drop( thing_data* thing, char_data* ch, thing_data* )
{
  ...
  // Cheat find //
  sprintf(temp, "%s drops %s.\n\r", ch->real_name( ), thing->Seen_Name(
NULL ));
  bug(temp);
...
void do_nostolen( char_data* ch, char* argument )
{
  obj_data*        obj;
  obj_data*      content;
  thing_data*    thing;
  ...
  for( int i = 0; i < obj->contents; i++ )
    if( ( content = object( obj->contents[i] ) ) != NULL )
       content->owner = ch->pcdata->pfile; 

//    if( (obj = object( thing) ) != NULL ){
      send(ch, "%s fixed.\n\r", obj->Seen_Name( NULL ));
//      set_owner( (obj_data*)thing, ch, NULL );
//    }
  }
