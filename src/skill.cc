...
char* consider_size_string ( int difference );
...
void do_consider( char_data* ch, char* argument )
{
  ...
  if( victim->species != NULL && victim->shdata->deaths == 0 ) {
    ...
    send( consider_string( ( double ) xp_compute( victim )
      /xp_compute( ch ) ), ch );
    send( consider_size_string( ( int ) ch->Size( ) - victim->Size( )), ch );
    }
  ...
  // added 10-5-00 by Rengeleif ... size in compair //
char* consider_size_string( int difference )
{
        // commented parts are more posible outcomes //
...
//   case -6: return " "; break;
//    case -5: return " "; break;
    case -4: return "Their foot is bigger than you are.\n\r"; break;
    case -3: return "You might be able to take out their knee.\n\r"; break;
    case -2: return "You come up to their waist.\n\r"; break;
    case -1: return "They could use you as an arm rest.\n\r"; break;

    case 0: return "They look about your size.\n\r"; break;

    case 1: return "You could use them as an arm rest.\n\r"; break;
    case 2: return "They come up to your waist.\n\r"; break;
    case 3: return "Be careful of bitten shins.\n\r"; break;
    case 4: return "Just step on them.\n\r"; break;
//    case 5: return " "; break;
//    case 6: return " "; break;
//    case 7: return " "; break;
//    case 8: return " "; break;
//    case 9: return " "; break;
    default: return "You cannot tell their exact size.\n\r";break;
    }
...
void do_inspect( char_data* ch, char* argument )
{
  ...
  if( ( obj = one_object( ch, argument, empty_string, &ch->contents,
			  ch->array) ) == NULL )
    {
...
void do_untrap( char_data *ch, char *argument )
{
  ...
  if( ( obj = one_object( ch, arg, empty_string, &ch->contents,
			  ch->array ) ) == NULL )
    {
  ...
  for( int i = 0; i < obj->contents; i++ ) 
    if( (trap = one_object( ch, argument, empty_string, &obj->contents )) 
	!= NULL &&
	trap->pIndexData->item_type == ITEM_TRAP && trap->Seen( ch ) )
      {
	found = TRUE;
...
void do_appraise( char_data *ch, char *argument )
{
  ...
  if( (obj = one_object( ch, argument, empty_string, &ch->contents )) == NULL)
    {
      send( ch, "You do not have that item.\n\r" );
      return;
    }
  
  if( IS_NPC( ch ) || ch->get_skill( SKILL_APPRAISE )== UNLEARNT ) {
  ...
  if( ch->check_skill( SKILL_BANDAGE ) )
    {
      ...
      act_neither(
		  "$n uses a $p to bandage $N's wounds stabilizing $S injuries.",
		  ch, victim, obj );
      send( victim, "Someone bandages your wounds.\n\r");
      victim->hit = 1;
      update_pos( victim );
  ...
  else
    {
      send( ch,
	    ...
	    victim );
      act_notchar( "$n tries to bandage $N's wounds but does more harm than good.",
	   ch, victim, ch );
    }
