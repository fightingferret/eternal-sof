  ...
  for( int i = 0; i < player_list; i++ ) {
    ...
    if( player->In_Game( )
      ...
      return( can_kill( ch, player, FALSE )
        || player->Befriended( ch ) ? obj : (thing_data*) NULL ); 
    }
...
void get_obj( char_data* ch, thing_array* array, obj_data* container )
{
  ...
  thing_func*     func  [ 6 ]  = { cant_take, forbidden,
                                   heavy, many, sat_on, gotten };
  content_array*      where;
  bool          run_trigger;
  thing_array  gotten_array;
  thing_data*         thing;
  obj_data*             obj;
  ...
  for( int i = 0; i < subset[5].size; i++ ) {
    thing = subset[5].list[i];
    ...
    if( run_trigger ) {
      if( get_trigger( ch, obj ) ) {
        if( ch->position == POS_EXTRACTED ) {
          delete array;
          if( container != NULL && container->Is_Valid( ) )
            container->Extract( );
          return;
          }
        if( !obj->Is_Valid( ) )
  ...
  page_priv( ch, &subset[2], "can't lift" );
  page_priv( ch, &subset[3], "can't handle" );
  page_priv( ch, &subset[4], "can't, someone is on" );
  page_publ( ch, &gotten_array, "get", container, "from" );
