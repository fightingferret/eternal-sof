  ...
  const char* dt, const char* loc_name )
{
  ...
  sprintf( tmp, "%s's %s %s%s%s your %s%c\n\r",
    ...
    dt, to_self( victim ), dam_name, c_normal( victim ),
    loc_name, punct );
  ...
  *tmp = toupper( *tmp );
  ...
  for( int i = 0; i < *ch->array; i++ ) {
    ...
    sprintf( tmp, "%s's %s %s%s%s %s%c",
      ch->Name( rch ), dt,
      damage_color( rch, ch, victim ), word, c_normal( rch ),
  ...
  const char* word, char punct )
{
  ...
  sprintf( tmp, "%s %s%s%s you%c\n\r", dt,
    to_self( victim ), word, c_normal( victim ), punct );
...
bool inflict( char_data* victim, char_data* ch, int dam, char* dt )
{
  ...
  if( dam > 1000 && ch->shdata->level < LEVEL_DEMIGOD ) {
  ...
  if( victim->rider != NULL && victim->position <= POS_RESTING ) {
    send( victim->rider,
