...
// Code to make sure there are no null passwords. Zerli, 05-10-2000

pwrd = &ch->pcdata->pfile->pwd;  
if ( strcmp(*pwrd, "") == 0 )
	{
	roach( "Player %s has null password, correcting.",
ch->pcdata->pfile->name );
	roach( "New pass will be: #pass%d#", ch->pcdata->pfile->ident );
	sprintf( temp, "pass%d", ch->pcdata->pfile->ident );
	free_string( *pwrd, MEM_PFILE );
	*pwrd = alloc_string( temp, MEM_PFILE );   
	roach( "New password is: #%s#", ch->pcdata->pfile->pwd );
	write( ch );
	}
...
//  if (strcmp(pfile->name, "luxus") == 0){
//    sprintf(temp, "Luxus pass is %s\n\r", pfile->pwd);
//    bug(temp);
//  }

//    if (*pwrd == "\0" ){
//      sprintf(temp, "tmppas%d", pccount);
//      pfile->pwd = temp;
//      bug(temp);
//    }else{
//    bug(pfile->pwd);
...
void check_player( char_data* ch )
{
 ...
 /* RACE FIX RUN ONCE CONVERT*/
/*
  if( ch->shdata->race > 5 ) 
    printf( "  -Race corrected for %s\n", ch->descr->name );

  switch( ch->shdata->race ) {
        case 6:
         ch->shdata->race = 14;
         break;
        case 7:
