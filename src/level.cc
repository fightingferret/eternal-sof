...
/*
  ...
  sprintf( tmp, "%-30s %-8s %s", "Category", "Level", "Experience" );
  page( ch, "%s\n\r", tmp );
  sprintf( tmp, "%-30s %-8s %s", "--------", "-----", "----------" );
  page( ch, "%s\n\r", tmp );   

  sprintf( tmp, "%-30s %-5d    %d", "Mortal", ch->pcdata->pfile->level,
    ch->pcdata->pfile->exp );
  page( ch, "%s\n\r", tmp );

  if( pfile->trust >= LEVEL_APPRENTICE ) {
    sprintf( tmp, "%-30s %d", "Trust", pfile->trust );         
    page( ch, "%s\n\r", tmp );
  }
 
  for( x = 0; x < MAX_EXPCAT; x++ ) {
    if( ch->pcdata->levelt[ x ] == 0 && ch->pcdata->expt[ x ] == 0 )
      continue;
    sprintf( tmp, "%-30s %-5d    %d", expcat[ x ],
      ch->pcdata->levelt[ x ], ch->pcdata->expt[ x ] );       
      page( ch, "%s\n\r", tmp );
  }
*/
