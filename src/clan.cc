...
inline void create_clan( pfile_data* pfile, char* name )
{
  *name = toupper( *name );
  ...
  title_data*  title  = new title_data( "Lord", pfile );
...
const char* create_msg = "Summoning a scribe daemon you request that the paperwork be drawn up to form a clan by the name of '%s'.  He politely nods his head, scribbles a few lines on the back of an envelope, hands you the envelope, and then informs you t




hat your bank account will be deducted 2000 sp for services rendered.";
...
void do_clans( char_data* ch, char* argument )
{
  ...
  if( is_set( &flags, 0 ) ) {
    if( pc->pcdata->pfile->clan != NULL ) {
      send( ch, "You cannot form clan if already a member of
one.\n\r" );
      return;
      }
    if( pc->shdata->level < 20 ) {
      send( ch, "You must be at least 20th level to form a clan.\n\r" );
      return;
      }
    if( pc->bank < 20000 ) {
      send( ch, "There is a 2000 sp charge, withdrawn from bank, for forming a clan and you lack\n\rsufficent funds.\n\r" );
...
void do_cedit( char_data* ch, char* argument )
{
  ...
  if( ( clan = ch->pcdata->pfile->clan ) == NULL ) {
    send( ch, "Cedit operates on the clan you are in and you aren't in a clan.\n\r" );
...
void do_allegiance( char_data* ch, char* argument )
{
  ...
  if( !consenting( victim, ch ) ) {
    fsend( ch, "Swearing allegiance to %s, who has not consented to you joining %s clan is pointless.", victim, victim->His_Her( ) );
...
/*    if( !strcmp( dp->d_name, "." ) || !strcmp( dp->d_name, ".." ) ) PUIOK */
    if( dp->d_name[0] == '.' )
      continue;
