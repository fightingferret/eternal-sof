  ...
  int level, int duration, int type )
{
  ...
  if( type == STYPE_QUAFF || type == STYPE_DRINK ) {
    ...
    fsend_seen( victim, "%s grasps %s throat and spasms in pain - %s does not survive long.", victim, victim->His_Her( ),
      victim->He_She( ) );
  ...
  if( type == STYPE_FILL ) {
    ...
    fsend( *victim->array, "%s quickly drops %s as %s dissolved by the liquid.", victim, obj, 
