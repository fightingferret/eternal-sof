...
bool        trigger_order    ( char_data*, char_data* );

char_data*  forcing_char = NULL;    /* 12/7/2000 PUIOK FIFO/LIFO*/
...
void do_report ( char_data* ch, char* argument )
{
  ...
  sprintf( tmp, "%s reports: %d/%dhp %d/%dmp %d/%dmv.", ch->real_name(),
    ch->hit, ch->max_hit, ch->mana, ch->max_mana, ch->move,
    ch->max_move );

  act_room( ch->in_room, tmp );
}
...
/*
  if( is_set( &ch->status, STAT_PET ) ) {
    if( ch->Is_Valid( ) )   PUIOK 16/7/2000
      interpret( ch, "shake" );
    return;
    }
*/
  ...
  if( victim->species != NULL && !( ch->species != NULL ) ) {
...
void add_follower( char_data* ch, char_data* victim, bool msg )
{
  ...
  if( ch == victim ) {
    bug( "Add_follower: attempt to add self to follower." );
    return;
  }
...
/* pet exp addition - zemus - april 2 */
/* formatting adjusted for cres - spyder - sept 12 2000 */
    sprintf( tmp,
      "[ %3d %-3s %-3s ] %-19s %9s %9s %9s %12d\n\r",
...
*/
  ...
  for( int i = 0; i < incognito; i++ ) {
    ...
    strcpy( name, incognito[i]->Seen_Name( ch ) );
    truncate( name, 26 );
    send( ch, "[   Incognito ] %-26s %s\n\r",
      name, condition_word( incognito[i] ) );
    }
}
...
bool trigger_order( char_data* ch, char_data* victim )
{
  mprog_data*    mprog;
