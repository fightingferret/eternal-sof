...
const char* oprog_flags [ MAX_OPROG_TRIGGER ][ 10 ] =
{
  { "" },
  { "" },
  { "" },
  { "" },
  { "when_wearing", "only_wearing", "" },
  { "" },
  { "" },
  { "" },
  { "" },
...
void do_opflag( char_data* ch, char* argument )
{
  obj_clss_data*    obj_clss;
  oprog_data*          oprog;
  wizard_data*        wizard;
  ...
  for( max = 0; *oprog_flags[oprog->trigger][max] != '\0'; max++ );

  if( max == 0 ) {
    send( ch, "That type of trigger has no oprog flags.\n\r" );
    return;
  }

  if( *argument == '\0' ) {
    display_flags( "Oprog", &oprog_flags[oprog->trigger][0],
      &oprog_flags[oprog->trigger][1], &oprog->flags, max, ch );
    return;
  }

  if( set_flags( &oprog_flags[oprog->trigger][0],
    &oprog_flags[oprog->trigger][1], &oprog->flags,
    max, NULL, ch, argument, FALSE, TRUE ) == NULL ) 
    return;
}

bool oflag_is_set( oprog_data* oprog, const char* keyword )
{
  for( int i = 0; *oprog_flags[oprog->trigger][i] != '\0'; i++ )
    if( !strcmp( oprog_flags[oprog->trigger][i], keyword ) )
...
#undef ot
  ...
  if( matches( argument, "value" ) ) { 
    oprog->value = atoi( argument );
    sprintf( buf, "Value on oprog set to %d.\n\r", oprog->value );
    send( buf, ch );
    return;
    }
