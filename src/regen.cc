...
void gain_condition( char_data* ch, int iCond, int value )
{
  ...
  // Added by Sphen (Sept 6, 00)
  if( ch->shdata->level >= LEVEL_APPRENTICE || value == 50 )
    return;
...
int Char_Data :: Hit_Regen( )
{
  ...
  if( position == POS_SLEEPING || position == POS_MEDITATING )
    ...
    regen = 3*regen/2;
/*
  if( is_set( &in_room->room_flags, RFLAG_INN ) )
    regen *= 2;
*/
  ...
  regen = regen/2 + hit_regen; // Added by Sphen (Sept 6, 00)
