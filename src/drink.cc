...
bool drink( char_data* ch, obj_data* obj )
{
  ...
  if( !is_set( &spell_table[spell-SPELL_FIRST].usable_flag, STYPE_DRINK ) ) {
    bug( "Drink: Unsupported spell." );
    bug( "--  Spell = %s", spell_table[spell-SPELL_FIRST].name );
    bug( "-- Liquid = %s", liquid_table[liquid].name );
    return TRUE;
    }

  ( *spell_table[spell-SPELL_FIRST].function )
    ( NULL, ch, NULL, 10, -1, STYPE_DRINK ); 
...
thing_data* already_full( thing_data* thing, char_data*, thing_data* )
{
  ...
  return( obj->value[1] >= obj->value[0] ? (thing_data*)NULL : obj );
}
...
bool react_filled( char_data* ch, obj_data* obj, int liquid )
{
  ...
  if( !is_set( &spell_table[spell-SPELL_FIRST].usable_flag, STYPE_FILL ) ) {
    bug( "Fill: Unsupported spell." );
    bug( "--  Spell = %s", spell_table[spell-SPELL_FIRST].name );
    bug( "-- Liquid = %s", liquid_table[liquid].name );
    return FALSE;
    }

  return( *spell_table[spell-SPELL_FIRST].function )
    ( NULL, ch, obj, 10, -4, STYPE_FILL );
}
...
thing_data* already_empty( thing_data* thing, char_data*, thing_data* )
{
  ...
  return( obj->value[1] != 0 ? thing : (thing_data*) NULL );
}
...
thing_data* fountain( thing_data* thing, char_data*, thing_data* )
{
  ...
  return( obj->value[0] != -1 ? thing : (thing_data*) NULL );
}
