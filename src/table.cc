...
const int table_abs_max [ MAX_TABLE ] =
{
  ...
  MAX_SOCIAL, MAX_SOCIAL, MAX_SOCIAL, MAX_SOCIAL, MAX_SOCIAL, MAX_SOCIAL,
  MAX_SOCIAL, MAX_SOCIAL, MAX_SOCIAL, MAX_SOCIAL, MAX_SOCIAL, MAX_SOCIAL,
  MAX_SOCIAL, MAX_SOCIAL, MAX_SOCIAL,  

  MAX_SPELL_ACT,
  MAX_LIQUID, MAX_SPELL, MAX_TOWN, MAX_SKILL, MAX_METAL, MAX_NATION,
  MAX_GROUP, MAX_RACE, MAX_PLYR_RACE, MAX_AFF_CHAR, MAX_AFF_OBJ, MAX_COMMAND,
  MAX_CMD_CAT, MAX_CLSS, MAX_CLSS+MAX_PLYR_RACE+1,
...
{
  sizeof( social_type ),  sizeof( social_type ),  sizeof( social_type ),
  sizeof( social_type ),  sizeof( social_type ),  sizeof( social_type ),
  sizeof( social_type ),  sizeof( social_type ),  sizeof( social_type ),
  sizeof( social_type ),  sizeof( social_type ),  sizeof( social_type ),
  sizeof( social_type ),  sizeof( social_type ),  sizeof( social_type ),
...
entry_data table_entry [ MAX_TABLE-MAX_PLYR_RACE ][ MAX_FIELD ] =
{
  ...
  { 
    ...
    { &skill_table[0].prac_cost[ CLSS_SAGE ],       VAR_INT   },
    { &skill_table[0].prac_cost[ CLSS_BLANK ],      VAR_INT   },
    { &skill_table[0].prac_cost[ CLSS_BLANK1 ],     VAR_INT   },
    { &skill_table[0].prac_cost[ CLSS_BLANK2 ],     VAR_INT   },
    { &skill_table[0].prac_cost[ CLSS_BLANK3 ],     VAR_INT   },
    { &skill_table[0].prac_cost[ CLSS_BLANK4 ],     VAR_INT   },
    { &skill_table[0].prac_cost[ CLSS_BLANK5 ],     VAR_INT   },
    { &skill_table[0].prac_cost[ CLSS_BLANK6 ],     VAR_INT   },
    { &skill_table[0].prac_cost[ CLSS_BLANK7 ],     VAR_INT   },
    { &skill_table[0].prac_cost[ CLSS_BLANK8 ],     VAR_INT   },
    { &skill_table[0].prac_cost[ CLSS_BLANK9 ],     VAR_INT   },
    ...
    { &skill_table[0].level[ CLSS_SAGE ],           VAR_INT   },
    { &skill_table[0].level[ CLSS_BLANK ],          VAR_INT   },
    { &skill_table[0].level[ CLSS_BLANK1 ],         VAR_INT   },
    { &skill_table[0].level[ CLSS_BLANK2 ],         VAR_INT   },
    { &skill_table[0].level[ CLSS_BLANK3 ],         VAR_INT   },
    { &skill_table[0].level[ CLSS_BLANK4 ],         VAR_INT   },
    { &skill_table[0].level[ CLSS_BLANK5 ],         VAR_INT   },
    { &skill_table[0].level[ CLSS_BLANK6 ],         VAR_INT   },
    { &skill_table[0].level[ CLSS_BLANK7 ],         VAR_INT   },
    { &skill_table[0].level[ CLSS_BLANK8 ],         VAR_INT   },
    { &skill_table[0].level[ CLSS_BLANK9 ],         VAR_INT   }
  },
...

  {
    ...
    { &nation_table[0].race[13],                    VAR_INT   },
    { &nation_table[0].race[14],                    VAR_INT   },
    { &nation_table[0].race[15],                    VAR_INT   },
...

  {
    ...
    { &command_table[0].options,                  VAR_TFLAG }
/*
...
const char* table_name [ MAX_TABLE ] = { 
  ...
  "Soc.Halfling", "Soc.Ent", "Soc.Titan", "Soc.Sprite",
  "Soc.Light", "Soc.Light1", "Soc.Light2", "Soc.Light3",
  "Soc.Light4", "Soc.Light5", "Soc.Centaur", "Soc.Lizard", "Soc.Firbolg",
  "Soc.Halfelf", "Soc.Neutral", "Soc.Neutral1", "Soc.Neutral2",
  "Soc.Neutral3", "Soc.Neutral4", "Soc.Neutral5",
  "Soc.Ogre", "Soc.Troll", "Soc.Orc", "Soc.Goblin", "Soc.Vyan",
  "Soc.Spriggan", "Soc.Duergar", "Soc.Pixie", "Soc.Evil",
  "Soc.Evil1", "Soc.Evil2", "Soc.Evil3", "Soc.Evil4",
  "Soc.Evil5", "Spell.Actions",
...
{
  social_fields, social_fields, social_fields, social_fields, social_fields,
  social_fields, social_fields, social_fields, social_fields, social_fields,
  social_fields, social_fields, social_fields, social_fields, social_fields,
  social_fields, social_fields, social_fields, social_fields, social_fields,
  social_fields, social_fields, social_fields, social_fields, social_fields,
  social_fields, social_fields, social_fields, social_fields, social_fields,
  social_fields, social_fields, social_fields, social_fields, social_fields,
  social_fields, social_fields, social_fields, social_fields,
...
inline char* entry_name( int table, int col )
{
  ...
  pntr = ((char*) table_entry[ table-MAX_PLYR_RACE ][0].offset)
...
{
  if( table <= MAX_PLYR_RACE ) 
    return (char**) &social_table[table][col]-(char**) &social_table[0][0]
      +((char**) table_entry[0][entry].offset);

  return ((char*) table_entry[ table-MAX_PLYR_RACE ][entry].offset)
    +col*table_size[table];
}
  ...
  switch( entry_type( i,k ) ) {
    ...
    case VAR_OBJ : {
      ...
      if( ( array = several_things( ch, argument, empty_string,
        &ch->contents ) ) == NULL ) {
        send( ch, "You don't have that object.\n\r" );
        return;
        }
      if( array->size != 1 ) {
        send( ch, "Only one type of object can be set per field.\n\r" );
        delete array;
        return;
        }

      obj_data* obj = object( array->list[0] );
      delete array;

      if( obj  == NULL ) {
        send( "The selection is not an object.\n\r", ch );
        return;
        }
      *(int*) pntr     = obj->pIndexData->vnum;
      *(int*) (pntr+1) = obj->selected; 
      send( ch, "Object set to %s.\n\r", obj );
      return;
      }
    ...
    case VAR_DICE : {
      class dice_field entry = {
        table_field[i][k], LEVEL_APPRENTICE, (int*) pntr };
      pntr = (void**) table_pntr( i,0,j );
...
/* -- PUIOK 7/3/2000 - do_tflag */

void do_tflag( char_data* ch, char* argument )
{
  wizard_data*         imm;
  int                table;
  int                entry;
  ...
  table  = imm->table_edit[0];
  entry  = imm->table_edit[1];
  
  display = *argument == '\0';

  if( !display && !edit_table( ch, table ) )
    return;
  
  for( i = 0; *table_field[table][i] != '\0'; i++ ) {
    if( entry_type( table, i ) == VAR_TFLAG ) {
      flag = (int*) table_pntr( table, i, entry );

      switch( table )
      {
        case TABLE_COMMAND :
          flag_list = cmd_table_flags;
          break;
      }
      
      for( max = 0; *flag_list[max] != '\0'; max++ ) ;
      ...
      if( max > 1 )
      {
        if( display )
        {
          if( found )
            page( ch, "\n\r" );
          display_flags( flag_list[0], &flag_list[1], &flag_list[2], flag,
            max - 1, ch );
        }
        else
          if( set_flags( &flag_list[1], &flag_list[2], flag, max - 1, NULL, ch,
            argument, FALSE, TRUE ) )
            return;
        found = TRUE;
      }
  ...
  if( !found ) 
    send( ch, "This table has no flags.\n\r" );
}
...
void sort_table( char_data* ch, int table )
{
  ...
  if( table > TABLE_SOC_VYAN  && table != TABLE_COMMAND 
    && table != TABLE_SPELL_ACT && table != TABLE_STARTING ) {
