...
/* added qflag command - zemus - july 16 */
void do_qflag( char_data* ch, char *argument )
{
  ...
  if( *argument == '\0' ) {
    ...
    for( i = 0; i < MAX_QUEST; i++ ) {
      switch( victim->pcdata->quest_flags[i] ) {
         case -1:
          qfs = 'C';
          break;
         case 0:
  ...
  if( i < 0 || i >= MAX_QUEST ) {
    send( ch, "Quest flag number out of range.\n\r" );
    return;
    }

  if( victim->pcdata->quest_flags[i] == 1 )
    victim->pcdata->quest_flags[i] = 0;
  else
    victim->pcdata->quest_flags[i] = 1;
 
  send( ch, "Quest flag %d on %s set to %s.\n\r", i,
    victim == ch ? "yourself" : victim->descr->name,
    victim->pcdata->quest_flags[i] == 1 ? "true" : "false" );
