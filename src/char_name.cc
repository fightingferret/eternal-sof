 ...
 *   INCOGNITO
 */
bool is_incognito( char_data* victim )
{
  pfile_data*    pfile;
  int            level;
...
const char* char_data :: Keywords( char_data* ch )
{
  ...
  if( ch != NULL && ( is_set( &ch->status, STAT_FORCED )
    || is_set( &ch->status, STAT_SOFTFORCED ) ) ) {
    char* tmp = static_string( );
    sprintf( tmp, "%s %s %s", descr->name,
      seperate( descr->singular, TRUE ),
      seperate( descr->keywords, TRUE ) );
    return tmp;
  }
...
const char* char_data :: Show( char_data* ch, int num )
{
  ...
  /* ADDED PUIOK 5/2/2000 */
  else if( position == POS_RESTING && pos_obj != NULL )
    sprintf( tmp, "%s %s here resting on %s.",
      Long_Name( ch, num ), num == 1 ? "is" : "are", pos_obj->Name( ch ) );
  
  else {
    switch ( position ) {
      case POS_DEAD:       string = "lying dead";          break;
      case POS_MORTAL:     string = "lying mortally wounded"; break;
      case POS_INCAP:      string = "lying incapacitated"; break;
      ...
      case POS_STANDING:
        if( fighting != NULL && !is_set( &status, STAT_LEAPING ) ) 
          sprintf( tmp, "%s %s here, fighting %s.",
            Long_Name( ch, num ), (num == 1 ? "is" : "are"),
            fighting == ch ? "YOU" : fighting->Name( ch ) );

        else if( species != NULL
          && !is_set( ch->affected_by, AFF_HALLUCINATE )
	  && !is_set( &status, STAT_PET ) && pos_obj == NULL ) 
  ...
  if( string != empty_string )
  {
    ...
    if( pos_obj != NULL )
    {
      if( position == POS_STANDING
        && is_set( affected_by, AFF_FLOAT ) )
        sprintf( sit_adj, " over %s", pos_obj->Name( ch ) );
      else
        sprintf( sit_adj, " on %s", pos_obj->Name( ch ) );
    }
    
    sprintf( tmp, "%s%s.", tmp, sit_adj );
  }
