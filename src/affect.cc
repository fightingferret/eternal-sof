...
void copy_affects( obj_clss_data* to, obj_clss_data* from )
{
  affect_data  *affect, *new_affect;
  
  for( int i = 0; i < from->affected; i++ ) {
    affect = from->affected[i];
    
    new_affect            = new affect_data;
    new_affect->type      = AFF_NONE;
    new_affect->duration  = -1;
    new_affect->location  = affect->location;
    new_affect->modifier  = affect->modifier;
    
    to->affected += new_affect;
  }
}
...
void do_affects( char_data* ch, char* argument )
{
  ...
  if( is_confused_pet( ch ) )
    return;
/* LEVEL_IMMORTAL CHANGED TO LEVEL_APPRENTICE   -- ZEMUS */
  ...
  for( int i = 0; i < MAX_ENTRY_AFF_CHAR; i++ ) 
    if( is_set( ch->affected_by, i )
      ...
      strcpy( tmp, source( ch, i ) );
      page( ch, "%-50s%s\n\r",
        aff_char_table[i].score_name, truncate( tmp, 30 ) );
      }
