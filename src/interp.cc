...
void interpret( char_data* ch, char* argument )
{
  ...
  time_data    time;
  char*         arg;
  bool   skip_progs = FALSE;
  bool  soft_forced;
  bool  afk_removed  = FALSE;
...
/* Skip trailing spaces */
  ...
  while( --arg >= argument && *arg == ' ');
  *++arg = '\0';

/* Remove AFK if not immortal */

  if( !is_set( &ch->status, STAT_SOFTFORCED )
    && !is_set( &ch->status, STAT_FORCED )
    && ch->shdata->level < LEVEL_APPRENTICE
    && is_set( &ch->status, STAT_AFK ) ) {
    bool was_page;
    
    if( !( was_page = is_set( &ch->status, STAT_PAGE_ALL ) ) )
...
/* Added by Zemus/Raiga Dec 20, 1999 updated PUIOK 25/12/1999*/
  ...
  if( *argument == '+' ) {
    ...
    else if( entries( ch->cmd_queue ) != 0 || ch->cast != NULL ) {
  ...
  if( !is_set( &ch->status, STAT_REMOTE ) && !afk_removed )
    clear_pager( ch );
  ...
  if( *argument == '_' && ch->shdata->level > LEVEL_APPRENTICE ) {
    if( *++argument == '\0' ) {
      next_page( ch->link );
      return;
    }
  ...
  soft_forced = is_set( &ch->status, STAT_SOFTFORCED );
  ...
  if( cmd < 0  || !has_permission( ch, command_table[cmd].level )
    || ( soft_forced
    && ( !is_set( &command_table[cmd].options, CMD_TFLAG_CODE_INTP )
    || ( is_set( &command_table[cmd].options, CMD_TFLAG_NO_WIZ )
    && ch->shdata->level >= LEVEL_APPRENTICE ) ) ) )
  {
    if( cmd < 0 )
      cmd = (-cmd)-1;
  ...
  if( found )
  {
    if( afk_removed && command_table[cmd].function == do_afk )
    {
      if( !is_set( &ch->status, STAT_REMOTE ) )
        next_page( ch->link );
      return;
    }
  }
  else
  {
    if( !check_social( ch, command, arg )
      && !speed_walking( ch, command ) && !soft_forced )
      send( ch, "<Type 'help' for help>\n\r" );
    
    if( afk_removed ) next_page( ch->link );
  ...
  if( is_set( command_table[cmd].level, PERM_DISABLED ) ) 
  {
    ...
    /* PUIOK 7/3/2000 */
  if( is_set( &command_table[cmd].options, CMD_TFLAG_QUEUE )
    && ch->active.time != -1 && !is_set( &ch->status, STAT_REMOTE ) ) 
    {
  ...
  if( command_table[cmd].position > POS_FIGHTING && ch->fighting != NULL ) {
      send( ch, "The current battle has you occupied.\n\r" );
      if( afk_removed ) next_page( ch->link );
      return;
    }
 
  if( ch->shdata->level < LEVEL_ENTITY ) {
    if( is_avatar( ch ) )
      immortal_log( ch, command, arg );
    else
      if( ch->pcdata != NULL && is_set( ch->pcdata->pfile->flags, PLR_LOGFILE ) )
        immortal_log( ch, command, arg );
    }

/*  if( command_table[cmd].reveal ) */
/*  PUIOK 7/3/2000 - tflags */
  if( is_set( &command_table[cmd].options, CMD_TFLAG_REVEAL ) )
    spoil_hide( ch );
  ...
  for( i = 0; i < ch->contents; i++ ) {
    if( ( obj = object( ch->contents[i] ) ) != NULL ) {
      for( oprog = obj->pIndexData->oprog; oprog != NULL;
        oprog = oprog->next ) {
        if( !oflag_is_set( oprog, "only_wearing" )
          && is_name( command, oprog->command, TRUE )
  ...
  for( i = 0; i < ch->wearing; i++ ) {
    if( ( obj = object( ch->wearing[i] ) ) != NULL ) {
      for( oprog = obj->pIndexData->oprog; oprog != NULL;
        oprog = oprog->next ) {
        if( ( oflag_is_set( oprog, "only_wearing" )
          || oflag_is_set( oprog, "when_wearing" ) )
          && is_name( command, oprog->command, TRUE )
          && ( oprog->target[0] == '\0' || ( argument[0] != '\0'
          && is_name( argument, oprog->target ) ) ) ) {
          disrupt_spell( ch );
...
void do_commands( char_data* ch, char* argument )
{
  ...
  if( *argument == '\0' ) {
    ...
    page( ch, "\n\r%s", j%4 != 0 ? "\n\r" : "" );
    page_centered( ch, "[ Type command <category> to see a list commands in that category. ]" );
