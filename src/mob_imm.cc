...
const char* plr_name [MAX_PLR] = { "Autoassist", "Autoexit",
  "Continuous", "Fifo.Order", "News.Chnnl", "Autosplit", "Brief",
"Chant",
  "Chat", "Ctell", "Email.Public", "Gossip", "Group.Incog", "Numeric",
  "Idle", "Autoprompt", "Info", "Lang.Id.", "??", "No.Summon", "Ooc",
  "Parry", "Pet.Assist", "Prompt", "Reverse", "Safe.Kill", "Say.Repeat",
  "Newbiechan","Searching", "Sneak", "Status.Bar", "Track",
  "Portal", "Atalk", "No.Chan.Soc", "Unused2", "No.Emote", "No.Shout",
  "No.Tell",
...
void do_allstat( char_data* ch, char* argument )
{
  char_data*       victim;
  wizard_data*        imm;
  ...
  if( ( victim = one_character( ch, argument, "allstat",
    ch->array, (thing_array*) &player_list ) ) == NULL )
    return;

  if( has_permission( imm, PERM_ALL_MOBS ) )
  {
    do_mstat( ch, argument );
    do_whois( ch, argument );
/*    do_identity( victim, "" );   Don't think this was intended - PUIOK */

    do_last( ch, argument );
    send( ch, "This command is under construction.  Trinity is going to" );
    send( ch, "put it into a table format without repeating information" );
    send( ch, "very soon!" );
  }
  else
  {
    send( ch, "Sorry, you can't use that command," );
    send( ch, "you need all mob editing permission" );
  }
...
void display( species_data* species, char_data* ch, char* buf, int& length )
{
  ...
  sprintf( &buf[length], "[%5d] %-30s %5d %5s %5d %5d\n\r",
    species->vnum, truncate( (char *) species->Name( ), 30 ),
...
void do_mfind( char_data* ch, char* argument )
{
  ...
  if( strlen( argument ) < 2 && spam_warning( ch ) )
    return;
...
/* Added by Zemus, 8-22-1999
   Copied and changed from Zeth's msetrange (=
*/
void do_mdelrange( char_data* ch, char* argument )
{
  ...
  if(!isdigit(*arg) || !isdigit(*arg2)) 
  {
    send(ch, "Usage: mdelrange <first vnum> <last vnum>\n\r");
    return;
  }
...
void do_msetrange( char_data* ch, char* argument )
{
  char 			arg [ MAX_INPUT_LENGTH ];
  char                  arg2 [MAX_INPUT_LENGTH ];
  char 			s [ MAX_INPUT_LENGTH ];
  ...
  argument=one_argument(argument,arg);
  argument=one_argument(argument,arg2);
  if(!isdigit(*arg) || !isdigit(*arg2) || !strlen(argument)) {
    send(ch, "Usage: msetrange <first vnum> <last vnum> <mset parameters>\n\r");
    return;
  }
...
void do_medit( char_data* ch, char* argument )
{
  ...
  if( exact_match( argument, "delete" ) ) {
    ...
    send( ch, "You delete mob %d.\n\r", species->vnum );
/* Added deletion messages - Zemus - Feb 27 */
    sprintf( buf, "Mobile deleted: %s (%s)", species->Name( ), ch->descr->name );
  ...
  if( isdigit( *argument ) ) {
    ...
    if( victim->species == NULL ) {
      if( ch != victim ) {
        if( is_immortal( victim ) && !has_permission( ch, PERM_ADMIN ) ) {
...
void do_mflag( char_data* ch, char* argument )
{
  ...
  int max [types] = { is_immortal( ch ) ? MAX_PLR : MAX_PLR_OPTION,
  ...
  int* flag_value [types] = { pc->pcdata->pfile->flags,
    pc->affected_by, imm == NULL ? (int) NULL : imm->permission };
  int uses_flag [types] = { -1, 1, has_permission( ch, PERM_RTABLES ) ? -1 : 1 };
...
#undef rn
  ...
  class string_field string_list[] = {
    { "keywords",      MEM_DESCR,(const char**)  &descr->keywords,    NULL },
    { "singular",      MEM_DESCR,(const char**)  &descr->singular,    NULL },
    { "",              0,          NULL,                NULL },   
    };
  ...
  if( species != NULL ) {
    class string_field string_list[] = {
      { "name",         MEM_DESCR,(const char**) &descr->name,          NULL },
      { "plural",       MEM_DESCR,(const char**) &descr->plural,        NULL },
      { "long_s",       MEM_DESCR,(const char**) &descr->long_s,        NULL },
      { "long_p",       MEM_DESCR,(const char**) &descr->long_p,        NULL },
      { "prefix_s",     MEM_DESCR,(const char**) &descr->prefix_s,      NULL },
      { "prefix_p",     MEM_DESCR,(const char**) &descr->prefix_p,      NULL },
      { "adj_s",        MEM_DESCR,(const char**) &descr->adj_s,         NULL },
      { "adj_p",        MEM_DESCR,(const char**) &descr->adj_p,         NULL },
      { "creator",      MEM_DESCR,(const char**) &species->creator,     NULL },
      { "",             0,          NULL,                  NULL },   
      };
    ...
    class int_field int_list[] = {
      ...
      { "price",               0,     10000000,  &species->price       },
...
#undef cn
    ...
    class dice_field dice_list[] = {
      { "movedice",    LEVEL_APPRENTICE,     &species->movedice },
      { "hitdice",     LEVEL_APPRENTICE,     &species->hitdice },    
      { "",            -1,            NULL }
      };
...
/* Changes: Zemus- Jan 1 */
      if( ( victim = one_character( ch, argument, "mstat", ch->array,
        (thing_array*) &player_list ) ) == NULL )
        return;

      if( victim != ch && !has_permission( ch, PERM_PLAYERS ) ) {
        send( ch, "You may not mstat players.\n\r" );
        return;
      }
/* End Changes */
  ...
  if( !is_set( &flags, 0 ) ) {
    ...
    if( pc == NULL )
      page( ch, "     Max Level: %-12d  Fame: %-5d\n\r", species->max_level, shdata->fame );
    else
      page( ch, "                %-12s  Fame: %-5d\n\r", "", shdata->fame );
