...
void do_chant( char_data* ch, char* argument )
{
  ...
  // Rengeleif, RP chants //
  if( !get_flags( ch, argument, &flags, "g", "Chant") ) 
    return;;
  // End of RP chants //
  ...
  for( int i = 0; i < player_list; i++ ) {
    pc = player_list[i];
    // Reng -- if line changed so imms hear all chants //
    if( (hear_channels( pc ) 
      && pc->pcdata->religion == ch->pcdata->religion) ||
      (hear_channels(pc) && has_permission( pc, PERM_IMM_CHAN)) ) {
      ...
      if( is_set( pc->pcdata->pfile->flags, PLR_CHANT ) ) {
        if( pc != player( ch ) ) {
          ...
          if( is_set( &flags, 0 ) && has_permission( ch, PERM_IMM_CHAN) ){
            
            sprintf( buf, "%s chants:", religion_table[ ch->pcdata->religion ].name );
            length = strlen( buf );
            sprintf( &buf[length], "%s%s", length < max_length ? "" : "\n\r", &tmp[length < max_length] );

          //End of Reng's Crap //

          }else{
            //Reng -- added so imms see all chants //
            if( has_permission( pc, PERM_IMM_CHAN) ){

              sprintf( buf, "%s chants[%s]:", ch->Seen_Name( pc ), religion_table[ ch->pcdata->religion ].name );
              length = strlen( buf );
              sprintf( &buf[length], "%s%s", length < max_length
              ? "" : "\n\r", &tmp[length < max_length] );
              //End of Reng crap//
...
/* added cangroup command zemus sept 12 */
void do_cangroup( char_data* ch, char* argument )
{
 int           flags;
 char_data*   victim;
 char            tmp [ MAX_STRING_LENGTH ];
 ...
 if( !get_flags( ch, argument, &flags, "r", "cangroup" ) )
   return;

 if( is_set( &flags, 0 ) ) {
   if( !has_permission( ch, PERM_PLAYERS ) ) {
     send( ch, "You do not have permission to do that.\n\r" );
     return;
      }
   victim = one_character( ch, argument, "cangroup", (thing_array*) &player_list );
   if( !victim )
     return;
   remove_bit( &victim->status, STAT_CANGROUP );
   send( ch, "Cangroup flag removed for %s.\n\r", victim->descr->name );
   }
 else {
   if( is_set( &ch->status, STAT_CANGROUP ) ) {
     remove_bit( &ch->status, STAT_CANGROUP );
     send( ch, "You are no longer marked as 'available to group'.\n\r" );
     sprintf( tmp, "%s is no longer marked as 'available to group'.", ch->descr->name );
     info( "", 0, tmp, IFLAG_CANGROUP, 1, ch );
     }
   else {
     if( !subtract_gsp( player( ch ), "Using Cangroup", 1 ) )
       return;
     set_bit( &ch->status, STAT_CANGROUP );
     send( ch, "You are now marked as 'available to group'.\n\r" );
     sprintf( tmp, "%s is now marked as 'available to group'.", ch->descr->name );
     info( "", 0, tmp, IFLAG_CANGROUP, 1, ch );
     }
   }
