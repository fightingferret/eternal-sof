...
const char* item_values [] = { "unused",
  ...
  "timer | next item | unused", "unused", "unused" };
...
bool is_same( obj_data* obj1, obj_data* obj2 )
{
  ...
  if( is_set( obj1->extra_flags, OFLAG_IN_USE )
    || is_set( obj2->extra_flags, OFLAG_IN_USE ) )
    return FALSE;
  ...
  /* PUIOK 15/5/2000 -- burning, dampness */
  if( obj1->burning != obj2->burning
    || obj1->burnmass != obj2->burnmass
    || obj1->burnlight != obj2->burnlight
    || obj1->dampness != obj2->dampness )
    return FALSE;
...
void set_owner( obj_data* obj, char_data* buyer, char_data* seller )
{
  ...
  if( obj->Belongs( seller ) && obj->pIndexData->item_type != ITEM_MONEY )
    obj->owner = ( ( buyer != NULL && buyer->pcdata != NULL )
      ? buyer->pcdata->pfile : (pfile_data*) NULL );
...
void condition_abbrev( char* tmp, obj_data* obj, char_data* ch )
{
  ...
  sprintf( tmp, "%s%s%s", i > 4 ? ( i > 7 ? blue( ch ) : green( ch ) ) 
    : ( i > 2 ? yellow( ch ) : red( ch ) ), abbrev[i], c_normal( ch ) );
...
const char* obj_data :: condition_name( char_data* ch, bool ansi )
{
  ...
  sprintf( tmp, "%s%s%s", i > 4 ? ( i > 7 ? blue( ch ) : green( ch ) ) 
    : ( i > 2 ? yellow( ch ) : red( ch ) ), txt, c_normal( ch ) );
...
obj_data* create( obj_clss_data* obj_clss, int number )
{
  ...
  for( int i = MAX_OFLAG; i < MAX_TEMPORARY_OFLAG; i++ )
    remove_bit( obj->extra_flags, i );  /* PUIOK 30/7/2000 */
  ...
  obj->age        = 0; 
  obj->rust       = 0;
  obj->timer      = obj_clss->timer;   /* 15/5/2000 PUIOK */

  /* PUIOK 15/5/2000 */
  obj->burnmass       = obj_clss->burnmass;
  obj->dampness       = obj_clss->dampness;
  obj->burnlight      = obj_clss->burnlight;
  ...
  switch( obj_clss->item_type ) {
    case ITEM_MONEY:
      ...
      break;
/*    case ITEM_FIRE:                       15/5/2000 - PUIOK - OBSELETE
    case ITEM_GATE:
      obj->timer = obj->value[0];
      break;
    case ITEM_LIGHT:
      obj->timer = obj->value[2];*/
    }
...
obj_data* duplicate( obj_data* copy, int num )
{
  ...
  /* PUIOK 15/5/2000 */
  obj->burnmass       = copy->burnmass;
  obj->dampness       = copy->dampness;
  obj->burning        = copy->burning;
  obj->burnlight      = copy->burnlight;
...
void drop_contents( obj_data* obj )
{
  ...
  if( obj->array == NULL
    || ( room = Room( obj->array->where ) ) == NULL )
    return;

  for( i = 0; i < obj->contents; i++ ) {
    if( ( content = object( obj->contents[i] ) ) == NULL )
      continue;
    if( content->timer == -2 )
      continue;
    
    content = (obj_data*) content->From( content->number );
    obj->owner = NULL;
    content->To( room );
    consolidate( obj );
  }
...
void replace_obj( obj_data* obj, obj_data* new_obj )
{
  char_data*     ch = NULL;
  content_array*  to_array;

  if( ( to_array = obj->array ) == NULL )
  {
    obj->Extract( );
    return;
  }
  
  if( ( ch = character( to_array->where ) ) != NULL )
    if( to_array == &ch->wearing )
    {
...
void load_objects( void )
{
  ...
  /* PUIOK 1/6/2000 */
  if( strncmp( header, "#OBJECTS", 8 ) )
    panic( "Load_objects: header not found" );

  version = atoi( &header[8] );
  ...
  for( ; ; ) {
    ...
    if( version >= 3 )
    {
      obj_clss->timer        = fread_number( fp );
      obj_clss->burnmass     = fread_number( fp );
      obj_clss->burnlight    = fread_number( fp );
      obj_clss->dampness     = fread_number( fp );
      
      if( version >= 4 )
        obj_clss->burnto     = fread_number( fp );
    }
