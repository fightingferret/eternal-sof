...
thing_data* Char_Data :: From( int )
{
  ...
  if( is_set( &status, STAT_DREAMWALKING ) )
  {
    *array -= this;
    array   = NULL;
  }
...
thing_data* Obj_Data :: From( int i )
{
  ...
  if( Room( where->where ) != NULL )
    for( int j = 0; j < *where; j++ )
      if( ( ch = character( where->list[j] ) ) != NULL )
        if( ch->pos_obj == this )
          ch->pos_obj = NULL;
...
void Char_Data :: To( content_array* where )
{
  ...
  if( !is_set( &status, STAT_REMOTE ) && !is_set( &status, STAT_DREAMWALKING ) )
    Thing_Data :: To( where );
  ...
  if( ( room = Room( where->where ) ) != NULL ) {
    ...
    // Code by Sphenotus.  Require sneak to move while hidden.
...
/*
  for( int i = 0; i < obj->array; i++ ) {
     if( character( obj->array[i] )->Can_See() ) {
       fsend( obj->array[i], "%s rots, and is quickly eaten by a grue.",  obj );
       }
     }
*/
