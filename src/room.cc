...
const char* rflag_name[ MAX_RFLAG ] = { "Lit", "Safe", "Indoors", "No.Mob",
  ...
  "No.Mount", "Arena", "Donation", "No.Pkill", "Stat.Corrected",
  "Bounty.Office" };
...
const terrain_type terrain [ MAX_TERRAIN ] =
{
  {  "Room",              1  },
  {  "Town Street",       1  },
  {  "Field",             2  },
...
void do_rdesc( char_data *ch, char *argument ) 
{
  ...
  if( imm->room_edit == NULL ) 
    ...
    imm->room_edit->text = edit_string( ch, 
      argument, imm->room_edit->text, MEM_EXTRA );
}
...
#define sn( i )   size_name[i]
  ...
  class type_field type_list [] = {
    ...
    { "terrain", MAX_TERRAIN,      
      (const char**) &tn(0), (const char**) &tn(1), &room->sector_type  },
...
void do_rstat( char_data* ch, char* )
{
  ...
  page( ch, "       Light: %-14d Weight: %.2f\n\r",
    room->Light(1), (float) room->contents.weight/100 );
  ...
  /* owner/weather added - zemus - april 11 */
  page( ch, "       Owner: %s\n\r", area->owner );
  page( ch, "     Weather: (see rset weather)\n\r" );
...
void do_rdel( char_data* ch, char*argument )
{
    room_data* room;
    char               buf   [ TWO_LINES ];
    char_data*         rch;
...
/*
    if( !can_extract( room, ch ) )
      return;
*/
    send( ch, "Rdel has been disabled, use 'redit delete room'.\n\r" );
/*    if( *argument == '\0' )
      {
      send( ch, "What room do you want to delete?\n\r" );
      return;
      }
    ...
    for( i = *ch->array-1; i >= 0; i-- ) 
       {
        rch = player( ch->array->list[i] );
        if( rch != NULL && rch != ch )   
          {
           send( rch, "The room you are in disappears.\n\r" );
           send( rch, "You find yourself floating in a fathomless void.\n\r" );
           rch->From( );
           rch->To( get_room_index( 1 ) );
          }
       }
     ...
     if( prev->vnum == vnum )
      ch->To( get_room_index( 1 ) );
     else     
      ch->To( prev );
 
    remove( room->area->room_first, room );

    sprintf( buf, "Room deleted: %d (%s)", vnum, ch->real_name() );
...
void do_redit( char_data *ch, char *argument )
{
  ...
  edit_extra( ch->in_room->extra_descr, imm, offset( &imm->room_edit,
    imm ), argument, "rdesc" );
}
...
room_data*   create_area    ( char_data*, int, const char* );

const char* rnew_no_arg = "Rnew requires no argument, the new room will be\
 created at the first available vnum in the area you're in.\n\r";
...
void do_rnew( char_data* ch, char* argument )
{
  ...
  if( *argument != '\0' ) {
    if( !has_permission( ch, PERM_ADMIN ) ) {
      fsend( ch, rnew_no_arg );
      return;
    }
    argument = one_argument( argument, arg );
    
    if( *argument == '\0' ) {
      send( ch, "Usage: Rnew <area vnum> <area file name>\n\r" );
      return;
    }
    if( ( vnum = atoi( arg ) ) == 0 ) {
      send( ch, "Invalid vnum.\n\r" );
      return;
    }
    argument = one_argument( argument, arg );

    if( ( room = create_area( ch, vnum, arg ) ) == NULL )
      return;
  }
  else if( ( room = create_room( ch ) ) == NULL )
    return;

  send( ch, "Room created at vnum %d and you're now in it.\n\r", room->vnum );
  fsend_seen( ch, "%s is taken to the room %s just created.\n\r",
    ch, ch->He_She( ) );
...
/* Dark when below Dimly Lit */

bool Room_Data :: is_dark( char_data* ch )
{
  if( Light( 1 ) < (int) exp( 3 ) - 1 )
  {
    if( ch == NULL )
      return TRUE;
