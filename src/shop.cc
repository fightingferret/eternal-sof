...
void shop_update( void )
{
  ...
  for( int i = 0; i < mob_list; i++ ) {
    ...
    if( !keeper->Is_Valid( ) || keeper->pShop == NULL
      || keeper->reset == NULL || player_in_room( keeper->in_room ) )
      continue;
...
bool will_trade( char_data* keeper, obj_data* obj )
{
  for( int i = 0; i < MAX_MATERIAL; i++ )
    if( is_set( &obj->pIndexData->materials, i ) )
      if( !is_set( &keeper->pShop->materials, i) )
        return FALSE;
...
thing_data* cantafford( thing_data* t1, char_data* ch, thing_data* t2 )
{
  ...
  return( remove_coins( ch, obj->temp, NULL ) ? obj : (thing_data*) NULL );
}
  ...
  thing_data* keeper )
{
  ...
  /* PUIOK 13/7/2000 - unconsolidated devalue fix */
  sold_obj = duplicate( obj, obj->selected );

  set_bit( sold_obj->extra_flags, OFLAG_IDENTIFIED );
  set_bit( sold_obj->extra_flags, OFLAG_KNOWN_LIQUID );

  if( sold_obj->pIndexData->item_type == ITEM_WAND
    || sold_obj->pIndexData->item_type == ITEM_STAFF ) 
    sold_obj->value[3] = sold_obj->pIndexData->value[3];

  free_string( sold_obj->label, MEM_OBJECT );
  sold_obj->label = empty_string;

  sold_obj->owner = NULL;
  sold_obj->To( keeper );
  consolidate( sold_obj );
...
{
  return( obj->contents == 0 ? obj : (thing_data*) NULL );
}


thing_data* plr_nosell( thing_data* thing, char_data*, thing_data* )
{
  obj_data* obj = object( thing );
  
  if( obj != NULL && is_set( obj->extra_flags, OFLAG_PLR_NO_SELL ) )
    return NULL;
...
void do_sell( char_data* ch, char* argument )
{
  ...
  thing_func*     func  [ 7 ]  = { in_use, cursed, stolen, uninterested,
                                   plr_nosell, not_empty, sold };

  sort_objects( ch, *array, keeper, 7, subset, func );

  for( int i = 0; i < subset[6]; i++ ) {
    obj = (obj_data*) subset[6][i];
    obj->Extract( obj->selected );
    add_coins( ch, obj->temp );
    }

  page_priv( ch, NULL, empty_string );
  page_priv( ch, &subset[2], "don't own" );
  page_priv( ch, &subset[3], "isn't interested in", keeper );
  page_priv( ch, &subset[4], "keep for yourself" );
  page_priv( ch, &subset[0], "are currently using" );
  page_priv( ch, &subset[1], "can't let go of" );
  page_priv( ch, &subset[5], NULL, NULL, "isn't empty", "aren't empty" );
  page_publ( ch, &subset[6], "sell", keeper, "to", "for" );
...
/*
 *   PLAYER NO SELL FLAGGING
 */


void do_nosell( char_data* ch, char* argument )
{
  thing_array* array;
  obj_data*      obj;
  int          flags;
  ...
  if( *argument == '\0' ) {
    for( int i = 0; i < ch->contents; i++ ) {
      if( ( obj = object( ch->contents[i] ) ) != NULL
        && is_set( obj->extra_flags, OFLAG_PLR_NO_SELL )
        && obj->Seen( ch ) )
        list += obj;
      }
    if( !is_empty( list ) )
      page_priv( ch, &list, "will not sell" );
    else {
      send( ch, "You have nothing in your inventory that you will\
 not try to sell.\n\r" );
      }
    return;
    }
  
  if( !get_flags( ch, argument, &flags, "r", "nosell" ) )
    return;
  
  if( ( array = several_things( ch, argument,
    (char*) ( flags == 0 ? "avoid selling" : "remove the hold on" ),
    &ch->contents ) ) == NULL )
    return;
  
  for( int i = 0; i < *array; i++ ) {
    if( ( obj = object( array->list[i] ) ) != NULL ) {
      obj = sunder( obj, obj->selected );
      if( flags != 0 )
        remove_bit( obj->extra_flags, OFLAG_PLR_NO_SELL );
      else
        set_bit( obj->extra_flags, OFLAG_PLR_NO_SELL );
      list += obj;
      }
    }
  
  page_priv( ch, &list, flags == 0 ? "will avoid selling" :
    "remove the hold on" );
...
void strip_nosell( obj_data* obj )
{
  obj_data* content;
...
void do_shedit( char_data* ch, char* argument )
{
  ...
  if( *argument == '\0' ) {
    ...
    sprintf( buf, "Shopkeeper: %s  [ Vnum: %d ]\n\r\n\r", ( species == NULL
  ...
  for( int i = 0; ; i++ ) {
    if( i >= *ch->array ) {
      if( is_set( &ch->in_room->room_flags, RFLAG_PET_SHOP )
        ...
        send( ch, "The shopkeeper is not around right now.\n\r" );
        return NULL;
        }
