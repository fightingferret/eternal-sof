...
/* gain_exp changed for pet gaining experience - zemus - april 2 */
...
/* add_exp changed for pet gaining experience - ZEmus - april 2 */
void add_exp( char_data* ch, int gain, const char* msg )
{
  ...
  for( ; ch->shdata->level > 1 && ch->shdata->level < LEVEL_APPRENTICE
    && ch->exp < 0; ) {
    lose_level( ch );
 ...
 if( ispet == FALSE ) {    
  if( ch->exp >= level && ch->shdata->level < LEVEL_APPRENTICE-1
    && ch->pcdata->quest_pts >= 0 && gain > 0 ) {
  
    if( ch->shdata->level >= 5 && !is_set( ch->pcdata->pfile->flags, PLR_APPROVED ) ) {
      send( ch, "To gain levels past 5th you must have an approved appearance.\n\r" );
      noexp = TRUE;
      }
   
    if( ch->pcdata != NULL && ch->shdata->level >= 1 ) {
      if( ch->pcdata->pfile->account == NULL ) {
        send( ch, "To gain levels past 1st you must be registered with a vaildated account.\n\r" );    
        noexp = TRUE;
        }
      else {
        if( ch->pcdata->pfile->account->email == empty_string ) {
          send( ch, "To gain levels past 5th you must be registered with a vaildated account.\n\r" );
          noexp = TRUE;
          }
        }
      }
...
/*
  if( ch->exp >= level && ch->shdata->level <= ch->species->max_level && gain > 0 ) {
    ch->exp -= level;
