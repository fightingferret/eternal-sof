...
//Quick cheater find //
  sprintf(temp, "%s gives %s to %s.\n\r", giver->Seen_Name( NULL ),
obj->Seen_Name( NULL ), receiver->real_name( ));
  bug(temp);


  strip_nosell( (obj_data*) obj );
...
void do_give( char_data* ch, char* argument )
{
  ...
  for( int i = *array-1; i >= 0; i-- )
    if( in_use( array->list[i], ch ) == NULL ) {
      use_set += array->list[i];
      *array -= array->list[i];
      }
  ...
  page_priv( ch, &use_set, "are currently using" );
