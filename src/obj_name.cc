...
void get_obj_adj( char* tmp, char_data* ch, obj_data* obj )
{
  ...
  */
  ...
  if( is_set( obj->extra_flags, OFLAG_GLOW )
    && ch != NULL && obj->Too_Dark( ch ) )
    sprintf( tmp+strlen( tmp ), "%sglowing", tmp[0] == '\0' ? "" : ", " ); 

  if( is_set( obj->extra_flags, OFLAG_HUM ) )
    sprintf( tmp+strlen( tmp ), "%shumming",
      tmp[0] == '\0' ? "" : ", " ); 

  if( is_set( obj->extra_flags, OFLAG_BURNING ) || obj->burning )
  ...
  if( obj->pIndexData->vnum == OBJ_BALL_OF_LIGHT ) 
    sprintf( tmp+strlen( tmp ), "%s%s", *tmp == '\0' ? "" : ", ",
      color_word[ range( 0, obj->timer/3, 10 ) ] );
...
bool char_data :: look_same( obj_data* obj1, obj_data* obj2 )
{
  ...
  if( obj1->pIndexData->vnum == 21
    && range( 0, obj1->timer/3, 10 ) != range( 0, obj2->timer/3, 10 ) )
    return FALSE;
  ...
  /* PUIOK 15/5/2000 - new burning */
  if( obj1->burning != obj2->burning )
    return FALSE;
  
  /* PUIOK 7/3/2000 - this != NULL check */
  if( this != NULL && IS_AFFECTED( this, AFF_DETECT_MAGIC )
...
const char* Obj_the_Name( obj_data* obj )
{
  const char* name;
  bool was_set = is_set( obj->pIndexData->extra_flags, OFLAG_THE );
  
  if( !was_set )
    set_bit( obj->pIndexData->extra_flags, OFLAG_THE );
  
  name = obj->Name( );

  if( !was_set )
    remove_bit( obj->pIndexData->extra_flags, OFLAG_THE );
