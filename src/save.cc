...
void  fix_owner       ( pfile_data*, thing_array& );
...
bool file_list( FILE* fp, save_entry* item, char* tmp, int memory )
{
  ...
  /* Other wise, read the item whose name matches that of tmp into the
     array */
  else {
    for( i = 0; item[i].name[0] != '\0'; i++ ) {
...
bool file_player( FILE* fp, player_data* player, char* tmp )
{
  save_entry list [] = {
    ...
    { "CoLR", 'I', 10,  &player->pcdata->color[45],         NULL  }, /* 1 June 2000 */
    { "Cond", 'I', 4,   player->pcdata->condition,          NULL  },
    { "Dths", 'I', 1,   &player->shdata->deaths,            NULL  },
    { "Exp.", 'I', 1,   &player->exp,                       NULL  },
    { "ExpT", 'I', 13,  player->pcdata->expt,               NULL  }, /* 1 June 2000 */
...
{
  save_entry list [] = {
    { "Bhtr", 'S', 0,   &pfile->hunter,        NULL  }, /* zemus 7-24 */
    { "Bnty", 'I', 1,   &pfile->bounty,        &zero }, /* 1 June 2000 */
    { "Crtd", 'I', 1,   &pfile->created,       NULL  },
    { "Hmpg", 'S', 0,   &pfile->homepage,      NULL  }, 
    { "Icq.", 's', 0,   &pfile->icq,           NULL  }, /* 1 June 2000 */
    { "Idnt", 'I', 1,   &pfile->ident,         NULL  },
    { "LsHt", 's', 1,   &pfile->last_host,     NULL  },
    { "LtOn", 'I', 1,   &pfile->last_on,       NULL  },
    { "Pknm", 'I', 1,   &pfile->pkcount,       NULL  },  /* NEWBC */
...
void write( player_data* ch )
{
  ...
  rename_file( EXTRA_DIR, ch->descr->name, EXTRA_PREV_DIR, ch->descr->name );

  /* This function dereferences a character .. basically it anhilates
     the ch->save_list array. */
  dereference( ch );

  /* Open up a file to store the player in. */
  if( ( fp = open_file( PLAYER_DIR, ch->descr->name, "w" ) ) == NULL ) 
    return;

  /* Set the values of the following variables correctly */
  played                      = ch->played;
...
bool load_char( link_data* link, char* name, const char* dir )
{
  ...
  if( !read_char( fp, imm, ch ) ) {
    ...
    roach( "-- Name: %s", name );
    if( strcmp( dir, PLAYER_DIR ) || ( fp = fopen( tmp, "r" ) ) == NULL || !read_char( fp, imm, ch ) ) 
      panic( "Load_Char: Prev file corrupted!" );
    roach( "Load_Char: Prev file intact." );
    }

  fix_owner( ch->pcdata->pfile, ch->contents );
  fix_owner( ch->pcdata->pfile, ch->locker );
  fix_owner( ch->pcdata->pfile, ch->wearing );
...
/*
 * Makes sure the owner of an object isn't someone who took over
 * a deleted player's former identity --  PUIOK 3/6/2000
 */

void fix_owner( pfile_data* pfile, thing_array& array )
{
  obj_data* obj;
  
  for( int i = 0; i < array; i++ ) {
    if( ( obj = object( array[i] ) ) != NULL && obj->owner != NULL )
      if( obj->owner->created >= pfile->last_on )
        obj->owner = NULL;
    fix_owner( pfile, array[i]->contents );
  }
}
...
bool file_object( FILE* fp, obj_data* obj, char* tmp )
{
  ...
  int     extra_flags  [ 2 ];
  int*    write_flags = obj->extra_flags;

  if( tmp == empty_string ) {
    write_flags = extra_flags;
    memcpy( extra_flags, obj->extra_flags, sizeof( int )*2 );
    for( int i = MAX_OFLAG; i < MAX_TEMPORARY_OFLAG; i++ )
      remove_bit( extra_flags, i );
    }

  save_entry list [] = {
    { "BrnM", 'I', 1, &obj->burnmass,    &oc->burnmass     }, /* 1 June 2000 */
    { "BrnL", 'I', 1, &obj->burnlight,   &oc->burnlight    }, /* 1 June 2000 */
    { "Brng", 'i', 1, &obj->burning,     &zero             }, /* 1 June 2000 */
    { "Cond", 'I', 1, &obj->condition,   NULL              },
    { "Cntn", 'O', 0, &obj->contents,    NULL              },
    { "Damp", 'I', 1, &obj->dampness,    &oc->dampness     }, /* 1 June 2000 */
    { "ExFl", 'I', 2, write_flags,       &oc->extra_flags  },
    { "Labl", 's', 0, &obj->label,       &empty_string     },
    { "Layr", 'I', 1, &obj->layer,       NULL              },
    { "Mat.", 'I', 1, &obj->materials,   &oc->materials    },
    { "Numb", 'I', 1, &obj->number,      &one              },
    { "Ownr", 'P', 0, &obj->owner,       NULL              }, /* 1 June 2000 */
...
/*This function anhilates the inputed characters save_list.  It deallocates
  the memory and clears the list */
void dereference( player_data* ch )
{
  /* array points to character save list */
  obj_array* array  = &ch->save_list;

  /* for all of the elements in the array, if the list element exists,
     set it's save pointer to NULL */
  for( int i = 0; i < array->size; i++ )
    if( array->list[i] != NULL )
      array->list[i]->save = NULL; 

  /* Deallocate and erase array */
  clear( *array );
...
bool file_pet( FILE* fp, char_data* pet, char* tmp )
{
  save_entry list [] = {
    ...
    { "Worn",  'O', 0,  &pet->wearing,    NULL     }, /* 1 June 2000 */
    { "",      ' ', 0,  NULL,             NULL     } };
...
void save_mobs( )
{
  ...
  /* Added a '2' for the version 2 file -- zemus [feb 18] */
  fprintf( fp, "#MOBILES2\n" );
  ...
  for( int i = 0; i < MAX_SPECIES; i++ ) {
    ...
    fprintf( fp, "\n#%d\n", species->vnum );
    fprintf( fp, "%s~\n", descr->name );
    /* added race by string - zemus [feb 18] */
...
/* shdata->race appeared after species->group in the next line- it was
replaced by the string at the top. zemus [feb 18] */
    fprintf( fp, "%d %d %d %d %d %d %d\n",
      species->nation, species->group,
...
void save_objects( )
{
  ...
  oprog_data*        oprog;
  int                    i;
  const int         version = 4;  /* June 1 2000 */
  ...
  for( i = 0; i < MAX_OBJ_INDEX; i++ ) {
    ...
    fprintf( fp, "%d %d %d %d %d\n",   /* Version 4 implementation */
      obj_clss->timer, obj_clss->burnmass, obj_clss->burnlight,
      obj_clss->dampness, obj_clss->burnto );
...
*/    
    ...
    for( oprog = obj_clss->oprog; oprog != NULL; oprog = oprog->next ) {
      fprintf( fp, "%d %d %d %d\n", oprog->trigger, oprog->obj_act == NULL ?
        ...
        oprog->value,
        oprog->flags );  /* Version 2 implementation */
      fprintf( fp, "%s~\n", oprog->command );
      fprintf( fp, "%s~\n", oprog->target );
      fprintf( fp, "%s~\n", oprog->code );
