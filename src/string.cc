...
/* This function takes an argument, a word, and a length.  If the arguement
   is the same as the word, and the length of the arguement is greater then
...
/* ADDED -- PUIOK 29/12/1999 */

char* withcolor_trunc( char* string, int length )
{
  char* letter;
  char* before[3] = { NULL, NULL, NULL };
  int visibles = 0;
  
  letter = string;
  
  while(*(letter = next_visible( letter, TRUE )) != '\0')
  {
    visibles++;
    
    if( visibles > length ) {
      strcpy( *before[2] == '@' ? before[2] - 1 : before[2], "@n..." );
      return string;
    }
    
    memmove( &before[1], &before[0], sizeof(char *) * 2 );
    before[0] = letter;
    letter++;
  }
...
char* next_visible( char* string, bool replace_indent_want_space )
{
  for( ; *string == '@' || *string == ' '; string++ ) {
    if(*string == ' ') {
      if(replace_indent_want_space)
        return string;
    } else
      switch( *++string ) {
        case 'I':
...
int sentence_length( const char* string )
{
  int count = 0;
  ...
  for( ; *string != '\0'; string++ )
  {
    ...
    if( ( *string == '.' || *string == '!' || *string == '?' ||
      *string == ';' ) && ( isspace( *( string + 1 ) ) || *( string + 1 ) == '\0' ) )
      return count;
  }
  return 0;
}
