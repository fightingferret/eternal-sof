...
const char* darkness_phrase [] =
{
  "Something moves in the darkness.\n\r",
  NULL,
  "Something moves in the darkness.\n\r",
  "Something flies through the darkness.\n\r",
...
bool char_data :: can_breath_underwater( )
{
  if( species != NULL )
    return (is_set( &species->act_flags, ACT_CAN_SWIM )
	    || is_set( affected_by, AFF_WATER_BREATHING ) );
  
  if( species == NULL )
    return is_set( affected_by, AFF_WATER_BREATHING );
  ...
  if( list != NULL ) {
    ...
    for( int i = 0; i < *list; i++ ) {
      ...
      if( follower->in_room == to_room ) {
        send( "\n\r", follower );
        show_room( follower, to_room, TRUE, NULL );
        }
      }
  ...
  int type, action_data* )
{
  ...
  for( int i = 0; i < room->contents; i++ )  
    if( ( rch = character( room->contents[i] ) ) != NULL
      ...
      else if( ch->Too_Dark( rch ) && darkness_phrase[type]
        && ch->Size( ) >= rch->Size( ) - 3 &&
        number_range( 0, max( 0, 1 + rch->Size( ) - ch->Size( ) ) ) == 0 )
        send( rch, darkness_phrase[type] );
      /*
...
{
  if( !ch->Seen( rch ) ) {
    if( ch->Too_Dark( rch ) && darkness_phrase[type]
      && ch->Size( ) >= rch->Size( ) - 3 &&
      number_range( 0, max( 0, 1 + rch->Size( ) - ch->Size( ) ) ) == 0 )
      send( rch, darkness_phrase[type] );
  ...
  if( is_set( &exit->exit_info, EX_CLOSED ) ) {
    ...
    if( rch != ch->mount )  /* PUIOK 13/2/2000 */
  ...
  for( int i = 0; i < *ch->array; i++ ) { 
    if( ( rch = character( ch->array->list[i] ) ) != NULL ) {
      ...
      if( includes( rch->aggressive, ch )
        ...
        ch->Size( )-2 ) && rch->position >= POS_STANDING && ( ( after
...
bool can_enter( char_data* ch, room_data* room )
{
  if( ch->pcdata != NULL && room->area->status != AREA_OPEN
    ...
    && !is_immortal( ch ) && !is_set( &ch->status, STAT_CLONED ) ) {
    send( ch, "That area is not open to players yet.\n\r" );
    return FALSE;
    }

  if( ch->Size( ) > room->size && !is_set( &ch->status, STAT_VAPOR ) ) {
...
bool passes_drunk( char_data* ch )
{
  ...
  if( ch->in_room != NULL )
    act_notchar( drunk_message[2*i+1], ch, (visible_data*) NULL, (visible_data*) NULL );
...
void do_search( char_data* ch, char* argument )
{
  ...
  if( toggle( ch, argument, "Searching",
    ...
    send( "[Searching increases movement point cost by 3 per move.]\n\r", ch );
