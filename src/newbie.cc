...
void  use_start_table    ( char_data*, const char* );
...
{
  send( link, "\n\rWhat you entered was unintelligible.  Please press return\
 and then select\n\rone of the choices given between the left and right\
 brackets.\n\r" );
  press_return( link );
...
bool check_parse_name( link_data* link, char* name )
{
 ...
 punctuation to stop the\n\rsegment of the population who insist on\
 the name --Starbright--.\n\r" );
    return FALSE;
 ...
 than dashes and\n\rapostrophes.\n\r" );
  ...
  for( int i = 0; i < MAX_SPECIES; i++ ) {
    ...
    if( is_name( name, species->descr->name ) ) {
      send( link, "\n\rThat is the name of monster in the game and thus unallowed.\n\r" );
...
void nanny_show_rules( link_data* link, char* )
{
  ...
  send( link, "\r\nDo you wish to continue and do you lawfuly
state that you over over the age of 13 years (in compliance
with COPA) (Y/N)? " );
...
void new_player( player_data* ch )
{
  ...
  set_bit( ch->pcdata->pfile->flags, PLR_CHAT );
  ...
  set_level( ch->iflag, MAX_IFLAG+NOTE_ANNOUNCEMENTS, 3 );
  ...
  use_start_table( ch, "default" );
  use_start_table( ch, clss_table[clss].name );
  use_start_table( ch, race_table[ch->shdata->race].name );
...
void use_start_table( char_data* ch, const char* name )
{
  for( int i = 0; i < MAX_CLSS+MAX_PLYR_RACE+1
    && starting_table[i].name != NULL; i++ )
    if( !strcasecmp( name, starting_table[i].name ) ) {
      use_start_table( ch, i );
      break;
    }
}
