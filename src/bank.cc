...
void do_deposit( char_data* ch, char* argument )
{
  ...
  if( is_number( argument ) ) {
    ...
    if( !add_bank( pc, amount ) ) {
      send( ch, "Your bank account has reached maximum.  Deposit denied.\n\r" );
      return;
      }
  ...
  thing_func*      func  [ 6 ]  = { in_use, cursed, stolen, corpse,
                                    no_room, deposit };
...
/* add bank function added - zemus - March 26 */
...
bool add_bank( char_data* ch, int amount )
{
 ...
 if( player( ch )->bank + amount < 0 ) {
   roach( "Negative bank balance check: %s", ch->descr->name );
   return FALSE;
   }

 player( ch )->bank += amount;
...
bool add_bank( player_data* pc, int amount )
{
 ...
 if( pc->bank + amount < 0 ) {
   roach( "Negative bank balance check: %s", pc->real_name() );
   return FALSE;
   }
