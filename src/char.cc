...
const char *size_name [ MAX_SIZE ] = {
  "Ant", "Rat", "Dog", "Gnome", "Human", "Horse",
  "Ogre", "Giant", "Elephant", "Dinosaur" };
  ...
  locker( this ), junked( this )
{
  ...
  vzero( pcdata->expt, MAX_EXPCAT );
  vzero( pcdata->levelt, MAX_EXPCAT );
  ...
  pcdata->clipboard     = empty_string;
  pcdata->clipboard     = alloc_string( pcdata->clipboard, MEM_CLIPBOARD );
...
Wizard_Data :: Wizard_Data( char* name ) : player_data( name ) 
{
  ...
  descdata_edit = NULL;  /* PUIOK 25/3/2000 */
  dd_last_seen  = 0;     /* PUIOK 25/3/2000 */
...
void update_max_move( char_data* ch )
{
  ...
  // Smith 3/4/2000
  if( ( move > ( 1 << 16 ) ) && ( ch->max_hit > ( 1 << 16 ) ) )
  {
    int ratio = ch->hit/(ch->max_hit >> 12);
    move = (move >> 12)*ratio;
  }
  else
