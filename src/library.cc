...
const cfunc_type cfunc_list [] =
{
  ...
  { "dam_message_acid",  &code_dam_message_acid,  nn, { ch, in, st, nn } },
  { "dam_message_cold",  &code_dam_message_cold,  nn, { ch, in, st, nn } },
  { "dam_message_fire",  &code_dam_message_fire,  nn, { ch, in, st, nn } },
  { "dam_message_shock", &code_dam_message_shock, nn, { ch, in, st, nn } },
  ...
  // added Rengeleif 10-5-00  for reputation //
  { "get_reputation",  &code_get_reputation,in, { ch, in, nn, nn } },
  ...
  { "is_sleeping",     &code_is_sleeping,   in, { ch, nn, nn, nn } },  
  { "is_standing",     &code_is_standing,   in, { ch, nn, nn, nn } },  
...
// added Rengeleif 10-5-00  //
void* code_get_reputation( void** argument )
{
  char_data*      ch = (char_data*) argument[0];
  int           repu = (int) argument[1];
  ...
  if( repu > MAX_ENTRY_NATION || repu < 1){
    code_bug( "Reputation: reputation out of bounds." );
    return NULL;
  }

  return (void*) pc->reputation.nation[repu];
...
// Sphenotus -- changed this function for that keyrings work with
//              has_obj()
...
void* code_has_obj( void** argument )
{
  ...
  for( int b = 0 ; b < ch->wearing ; b++ )
    if( ( obj = object( ch->wearing[b] ) ) != NULL )
      if( obj->pIndexData->vnum == 56 )
        if( find_vnum( obj->contents, vnum ) ) return( void* ) find_vnum(
obj->contents, vnum );
...
void* code_obj_type( void** argument )
{
  obj_data*     obj = (obj_data*) argument[0];
