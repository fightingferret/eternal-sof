...
const char* prompt_complex = 
  "?p' -- MORE -- '<%f?f|%hhp %ee ?m'[%mmv]'!m'%vmv' %gwm %xxp?l' %C' %d?b' %c'> ";
...
void prompt_flags( char* flags, char_data* ch )
{
  bool request = ( !is_empty( request_app ) 
    ...
    || ( !is_empty( request_imm ) && is_immortal( ch ) );
...
void prompt_ansi( link_data* link )
{
  ...
  if( ch->shdata->level >= LEVEL_APPRENTICE ) {
    ...
    sprintf( tmp,
      ...
      bold_red_v( ch ), ch->hit, ch->max_hit, c_normal( ch ),
      blue( ch ), ch->mana, ch->max_mana, c_normal( ch ),
      green( ch ), ch->move, ch->max_move, c_normal( ch ),
  ...
  for( i = 0, j = 0; i < length; i++ ) {
    ...
    if( input[i] == '%' ) {
      switch( input[++i] ) {
       ...
       /* added V flag zemus sept 14 */
       case 'r' :
       if( ch->in_room != NULL )
         sprintf( &output[j], "%d", ch->in_room->vnum );
       else
         sprintf( &output[j], "[NULL]" );
       break;
