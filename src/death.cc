...
/* Bounty code changed/added july 14 - zemus */
void register_death( char_data* victim, char_data* ch, char* dt )
{
  ...
  if( victim->species != NULL ) {
    ...
    vnum = victim->species->vnum;
    for( mprog = victim->species->mprog; mprog != NULL; mprog = mprog->next )
      if( mprog->trigger == MPROG_TRIGGER_DEATH ) {
        if( !victim->Is_Valid( ) ) {
          roach( "Death Mpcode: Mobile already junked." );
          roach( "-- Vnum = %d", vnum );
          return;
          }
        var_mob  = victim;
        var_ch   = ( ch == NULL || ch->Is_Valid( ) ) ? ch : ( ch = NULL );
...
/* Bounty */
  strcpy( tmp3, "." );
  if( ch != NULL && victim->pcdata != NULL
    && ( bounty = victim->pcdata->pfile->bounty ) > 0
    && victim != ch && ( pc = player( ch ) ) != NULL ) {
    fsend( ch, "A bounty daemon runs up to you and mutters something about\
 %s, %d cp, and your bank account.\n\r", victim->Name ( ), bounty );
    sprintf( tmp3, ", who receives a payment of %d cp.", bounty );
    pc->bank += bounty;
    victim->pcdata->pfile->bounty = 0;
    }

  sprintf( tmp1, "%s killed by %s%s", victim->Name( ),
    ch == NULL ? dt : ch->Name( ), tmp3 );
  sprintf( tmp2, "%s killed by %s at %s%s", victim->Name( ),
    ch == NULL ? dt : ch->Name( ), victim->array->where->Location( ), tmp3 );
  ...
  if( ch != NULL && ch->pcdata != NULL ) {
    ch->pcdata->pfile->pkcount++;
    sprintf( tmp1, "Pkilled %s at %s.", victim->real_name( ),
      victim->array->where->Location( ) );
    player_log( ch, tmp1 );
    }
}
...
void death_cry( char_data* ch )
{
  room_data*  room;
  char*        msg;
  char*        cry1 = "You hear something's death cry.\n\r";
  char*        cry2 = "You hear someone's death cry.\n\r";
  
 
  if( ( room = Room( ch->array->where ) ) == NULL )
    return;

  msg = ch->species ? cry1 : cry2;
...
void loot_corpse( obj_data* corpse, char_data* ch, char_data* victim )
{
  ...
  if( level == 1 || ( ch->species != NULL
    && !is_set( &ch->species->act_flags, ACT_CAN_CARRY ) ) ) {
    send_priv( ch, &corpse->contents, "contains", corpse ); 
    return;
  }  

  if( ( array = several_things( ch, "all", empty_string,
    &corpse->contents ) ) == NULL )
    return;
      ...
      if( obj->pIndexData->item_type == ITEM_MONEY ) {
        for( int j = 0; j < MAX_COIN; j++ )
          if( obj->pIndexData->vnum == coin_vnum[j] )
            amount += obj->number*coin_value[j];
      }
      else if( level != 3 )
        *array -= obj;
    }
  
  if( !( was_set = is_set( ch->pcdata->pfile->flags, PLR_CONTINUOUS ) ) )
    set_bit( ch->pcdata->pfile->flags, PLR_CONTINUOUS );

  get_obj( ch, array, corpse );

  if( !ch->Is_Valid( ) )
    return;
  
  if( !was_set )
    remove_bit( ch->pcdata->pfile->flags, PLR_CONTINUOUS );
  
  if( corpse->Is_Valid( ) && corpse->contents > 0 )
    send_priv( ch, &corpse->contents, "still contains", corpse ); 

  if( is_set( ch->pcdata->pfile->flags, PLR_AUTO_SPLIT ) ) 
    split_money( ch, amount, FALSE );
  else if( player( ch ) != NULL )
    ( (player_data*) ch )->split_hold += amount;
}
...
/* added by zemus aug 10 1999 */
  if( victim->shdata->level >= LEVEL_APPRENTICE)
   {
    send( ch, "You may not slay immortals.\n\r");
    return;
   }
