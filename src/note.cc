...
const char* noteboard_name [ MAX_NOTEBOARD ] = { "general", "immortal",
  "ideas",
  "bugs", "not_bug", "announcements", "information", "stories",
"current",
  "wanted", "fixed", "code", "avatar", "clan", "builder", "development",
  "quest" };
...
{
  if( board == NOTE_IMMORTAL
    || board == NOTE_BUILDER || board == NOTE_DEVELOPMENT || board ==
NOTE_CODE )
...
bool can_remove( char_data* ch, note_data* note )
{
  ...
  if( note->noteboard == NOTE_BUGS && is_immortal( ch ) )
    return TRUE;
...
{
  if( !is_immortal( ch ) && ( i == NOTE_IMMORTAL
...
void do_notes( char_data* ch, char* argument )
{
  ...
  if( ( note = pc->note_edit ) == NULL ) {
    ...
    if( matches( argument, "move" ) ) {
      ...
      info( empty_string, max( wizinvis, noteboard_level( noteboard ) ),
        tmp1, IFLAG_NOTES, 2, ch );
...
*/
      ...
      info( empty_string, max( wizinvis, noteboard_level( noteboard ) ),
        tmp1, IFLAG_NOTES, 3, ch, 
        noteboard == NOTE_CLAN ? ch->pcdata->pfile->clan : (clan_data*) NULL );
      extract( ch, note );
      save_notes( noteboard, clan );
  ...
  if( !strncasecmp( "title ", argument, 6 ) ) {
    ...
    if( strlen( argument ) == 1 && *argument == '*' )
    {
      const int max_title_length = 40;
      char new_title[max_title_length + 1];

      int old_length = strlen( note->title );
      
      if( old_length >= max_title_length )
        strcpy( &note->title[40 - 3], "..*" );
      else
      {
        sprintf( new_title, "%s*", note->title );
        if( !acceptable_title( ch, new_title ) )
        {
          bug( "Revise max_title_length in do_notes, note.cc" );
          return;
        }
        free_string( note->title, MEM_NOTE );
        note->title = alloc_string( new_title, MEM_NOTE );
      }
      send( ch, "Title changed to: %s\n\r", note->title );
      return;
    }
...
const char* message1 =
  "The mail daemon takes the letter, opens it, reads it carefully, rolls\
...
const char* message3 =
  "You give a mail daemon your letter.  The mail daemon loiters nearby and whistles a\
  tune.  When he realizes you aren't going to pay postage, he stamps around just out of\
  reach and mumbles rudely about immortals, before slowly wandering off in the wrong direction.";

const char* message4 =
  "You give a mail daemon your letter.  He stands nearby with his hand out for\
 a few minutes and then grumbles and wanders off while chewing on a corner of the letter.";

const char* dream_msg =
  "You really don't want to invite a daemon into your dreams.\n\r";


/* added unread message status - zemus - march 21 */
void mail_message( char_data* ch )
{
  ...
  for( note = ch->pcdata->pfile->mail; note != NULL; note = note->next ) {
    ...
    if( unread != 0 ) {
      sprintf( tmp, "You have %s unread mail message%s.",
        number_word( unread ), unread == 1 ? "" : "s" );
      send_centered( ch, tmp );
      }
...
void display_mail( char_data* ch )
{
  ...
  for( note = ch->pcdata->pfile->mail; note != NULL; note = note->next ) {
    sprintf( tmp, "[%c%2d] %-40s %-15s %s\n\r",
      note->status == 0 ? '*' : ' ', ++line, note->title,
...
*/
  ...
  /* PUIOK 6/3/2000 - clones support */

  for( link = link_list, clones = 0; clones < player_list; ) {
    if( link == NULL ) {
      mch = player_list[ clones++ ];
      if( !mch->In_Game( ) || mch->link == NULL
        || !is_set( &mch->status, STAT_CLONED ) )
        continue;
    }
    else {
...
void read_mail( pfile_data *pfile )
{
  ...
  if( ( fp = fopen( tmp, "r" ) ) != NULL ) {
    ...
    strcpy( header, fread_word( fp ) );
    if( strcmp( header, "#MAIL" ) ) 
      panic( "Load_mail: header not found" );
