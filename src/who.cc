...
void do_who( char_data* ch, char* argument )
{
  ...
  if( !get_flags( ch, argument, &flags, "caikubp", "who" ) )
    return;
    ...
    *buf = '\0';
    ...
    for( link = link_list, clones = 0; clones < player_list; ) {
      ...
      switch( type ) {
         case 0:  /* immortals */
           if( wch->shdata->level < LEVEL_APPRENTICE )
             continue;
           if( is_set( &flags, 0 ) || is_set( &flags, 3 ) || is_set( &flags, 4 ) || is_set( &flags, 5 ) || is_set( &flags, 6 ) )
             continue;
           if( is_set( &flags, 1 ) && !is_set( &wch->status, STAT_AFK ) )
             continue;
           break;
         case 1:  /* befriended */
           if( wch->shdata->level > LEVEL_APPRENTICE )
             continue;
           if( !ch->Befriended( wch ) )
             continue;
           if( is_set( &flags, 2 ) || is_set( &flags, 3 ) || is_set( &flags, 4 ) ) 
             continue;
           if( is_set( &flags, 0 ) && !is_set( &wch->status, STAT_CANGROUP ) )
             continue;
           if( is_set( &flags, 1 ) && !is_set( &wch->status, STAT_AFK ) )
             continue;
           break;
         case 2:  /* known */
           ...
           if( ch->Befriended( wch ) )
             continue;
           if( !ch->Recognizes( wch ) )
             continue;
           if( ch == wch )
             continue;
           if( is_set( &flags, 2 ) || is_set( &flags, 4 ) || is_set( &flags, 5 ) )
             continue;
           if( is_set( &flags, 0 ) && !is_set( &wch->status, STAT_CANGROUP ) )
             continue;
           if( is_set( &flags, 1 ) && !is_set( &wch->status, STAT_AFK ) )
         ...
         case 3: /* unknown */
           ...
           if( ch->Recognizes( wch ) )
             continue;
           if( is_set( &flags, 2 ) || is_set( &flags, 3 ) || is_set( &flags, 5 ) )
             continue;
           if( is_set( &flags, 0 ) && !is_set( &wch->status, STAT_CANGROUP ) )
             continue;
           if( is_set( &flags, 1 ) && !is_set( &wch->status, STAT_AFK ) )
      ...
      if( !found ) {
        ...
        switch( type ) {
          case 0: 
            ...
            break;
          case 1:
            page_title( ch, "Befriended" );
            break;
          case 2:
            page_title( ch, "Known" );
            break;
          case 3:
            page_title( ch, "Unknown" );
            break;
          }
        found = TRUE;
        }  
      ...
      if( wch->shdata->level >= LEVEL_APPRENTICE ) {
        ...
        if( clan != NULL && get_title( wch->pcdata->pfile ) && knows_members( ch, clan ) ) 
          sprintf( avspot, "CL" );
        else
          sprintf( avspot, "  " );
        if( is_avatar( wch ) )
        ...
        if( is_incognito( wch, ch ) ) {
          sprintf( buf, "[  ???   %-3s %-2s ]",
            race_table[wch->shdata->race].abbrev, avspot );
        }
        else {
          sprintf( buf, "[ %2d %-3s %-3s %-2s ]", wch->shdata->level,
      ...
      if( imm == NULL ) {
        if( clan != NULL && knows_members( ch, clan ) )
          sprintf( extras, clan->abbrev );
        }
      else {
        if( imm->wizinvis > 0 && is_set( wch->pcdata->pfile->flags, PLR_WIZINVIS ) )
...
/*
  ...
  fprintf( fp, "<html>\n" );
  fprintf( fp, "<title>Players on The Summers of Faldyor</title>\n" );
...
void do_icq( char_data* ch, char* argument )
{
  ...
  if( pfile->icq == empty_string )
    send( ch, "%s hasn't specified an icq number.\n\r", pfile->name );
  else
    send( ch, "%s's icq is %s.\n\r", pfile->name, pfile->icq ); 
}

// Rengeleif, Religion who //
...
void do_rwho( char_data* ch, char* argument ){
  ...
  if(!has_permission( ch, PERM_IMM_CHAN))
    return;

  for(rel = 0; rel <= MAX_RELIGION; rel++){
    ...
    for( link = link_list, clones = 0; clones < player_list; ) {
      ...
      count++; 
       if( wch->pcdata->religion == rel ){   
         sprintf( tmp, "%17s%17s\n\r", wch->real_name( ),
                                      religion_table[ rel ].name); 
         page( ch, tmp );
       }
    }
  }
